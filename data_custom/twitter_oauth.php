...
if (!isset($response['oauth_token'])) warn_exit('Twitter authorisation failed: '.$response['message']);
set_value('twitter_token',$response['oauth_token']);
set_value('twitter_token_secret',$response['oauth_token_secret']);
inform_exit('Twitter integrated. Now future postings (not the one you just did) will syndicate.');