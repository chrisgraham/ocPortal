...
if ($type=='')
{
	echo <<<END
		...
		<p>This tool will not find out if areas of your site have been vandalised, because there is way too much information to scan and present for that to be viable. Instead, it focuses on compromised file systems, ocPortal permissions, and database-stored PHP/OcCLE-Code. It only checks staff settings on the local OCF, not on any other third-party forum.</p>
...
END;
	...
	echo <<<END
			<input class="buttons__proceed button_page" type="submit" value="Begin" />
		</div>
END;
} else
{
