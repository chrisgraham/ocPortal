...
class Module_cms_galleries extends standard_crud_module
{
	...
	function __gimp()
	{
		...
		return $this->cat_crud_module->_do_next_manager($this->title,do_lang_tempcode('GIMP_SUCCESS_EDIT_TITLES_INLINE',escape_html(integer_format(count($media_imported)))),$cat);
	}
	...
	function get_form_fields($id=NULL,$title='',$cat='',$description='',$url='',$thumb_url='',$validated=1,$allow_rating=NULL,$allow_comments=NULL,$allow_trackbacks=NULL,$notes='',$adding=true)
	{
		...
		if ((get_value('no_confirm_url_spec_cats')!=='1') || ($cat==''))
		{
			...
			if (empty($root_cat) && $GLOBALS['SITE_DB']->query_select_value('galleries','accept_images',array('name'=>'root'))==0) $root_cat='root'; // Don't show 'root' itself
...
class Module_cms_galleries_alt extends standard_crud_module
{
	...
	function get_form_fields($id=NULL,$title='',$cat='',$description='',$url='',$thumb_url='',$validated=1,$allow_rating=NULL,$allow_comments=NULL,$allow_trackbacks=NULL,$notes='',$video_length=NULL,$video_width=NULL,$video_height=NULL)
	{
		...
		if ((get_value('no_confirm_url_spec_cats')!=='1') || ($cat==''))
		{
			...
			if (empty($root_cat) && $GLOBALS['SITE_DB']->query_select_value('galleries','accept_videos',array('name'=>'root'))==0) $root_cat='root'; // Don't show 'root' itself
			$fields->attach(form_input_tree_list(do_lang_tempcode('GALLERY'),do_lang_tempcode('DESCRIPTION_GALLERY'),'cat',$root_cat,'choose_gallery',$filters,true,$cat,false,NULL,false,$gallery_title));
		} else
		{
			$hidden->attach(form_input_hidden('cat',$cat));
		...
		if ((is_null($id)) && (is_null($cat)))
		{
			return do_next_manager($title,$description,
				...
				array_merge($extra,array(
					array('menu/cms/galleries/add_one_image',array('_SELF',array('type'=>'ad'),'_SELF'),do_lang('ADD_IMAGE')),
					array('menu/cms/galleries/add_one_video',array('_SELF',array('type'=>'av'),'_SELF'),do_lang('ADD_VIDEO')),
