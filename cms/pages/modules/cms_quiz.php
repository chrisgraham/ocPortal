...
class Module_cms_quiz extends standard_aed_module
{
	...
	function fill_in_edit_form($_id)
	{
		...
		foreach ($question_rows as $i=>$q)
		{
			...
			$text.=/*strval($i+1).') '.*/get_translated_text($q['q_question_text']).(($q['q_long_input_field']==1)?' [LONG]':'').((($q['q_num_choosable_answers']==count($answer_rows)) && ($q['q_num_choosable_answers']!=0))?' [*]':'').chr(10);
