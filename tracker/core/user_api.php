...
function user_cache_row( $p_user_id, $p_trigger_errors = true ) {
	...
	$row= Array (
		...
		'email'=>$ocp_row['m_email_address'],
		'password'=>$ocp_row['m_pass_hash_salted'].':'.$ocp_row['m_pass_salt'].':'.$ocp_row['m_password_compat_scheme'],
	...
	$query = "REPLACE INTO $t_user_table
				    ( id, username, email, password, date_created, last_visit,
				     enabled, access_level, login_count, cookie_string, realname, protected, lost_password_request_count, failed_login_count )
				  VALUES
				    ( " . db_param() . ', ' . db_param() . ', ' . db_param() . ', ' . db_param() . ', ' . db_param() . ', ' . db_param()  . ",
				     " . db_param() . ',' . db_param() . ',' . db_param() . ',' . db_param() . ',' . db_param() . ',' . db_param() . ',' . db_param() . ', ' . db_param() . ')';
	db_query_bound( $query, Array( $row['id'], $row['username'], $row['email'], $row['password'], $row['date_created'], $row['last_visit'], $row['enabled'], $row['access_level'], $row['login_count'], $row['cookie_string'], $row['realname'], $row['protected'], $row['lost_password_request_count'], $row['failed_login_count'] ) );
