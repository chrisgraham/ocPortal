...
# along with MantisBT.  If not, see <http://www.gnu.org/licenses/>.
	...
	if ( $f_master_bug_id > 0 ) {
		...
		$t_project_id			= gpc_get_int( 'project_id', $current_project );

		$t_changed_project		= false;/// don't enable due to mantis bug helper_get_current_project()!=$t_project_id;
	}

	$f_report_stay			= gpc_get_bool( 'report_stay', access_has_project_level( config_get( 'roadmap_update_threshold' )) );

	$t_fields = config_get( 'bug_report_page_fields' );
	$t_fields = columns_filter_disabled( $t_fields );

	$simple			= gpc_get_int( 'simple', 0 );

	$tpl_show_category = in_array( 'category_id', $t_fields );
	$tpl_show_reproducibility = ($simple==0) && in_array( 'reproducibility', $t_fields );
	$tpl_show_severity = ($simple==0) && in_array( 'severity', $t_fields );
	$tpl_show_priority = ($simple==0) && in_array( 'priority', $t_fields );
	$tpl_show_steps_to_reproduce = ($simple==0) && in_array( 'steps_to_reproduce', $t_fields );
	$tpl_show_handler = ($simple==0) && in_array( 'handler', $t_fields ) && access_has_project_level( config_get( 'update_bug_assign_threshold' ) );
	$tpl_show_profiles = ON == config_get( 'enable_profiles' );
	$tpl_show_platform = ($simple==0) && in_array( 'platform', $t_fields );
	$tpl_show_os = ($simple==0) && in_array( 'os', $t_fields );
	$tpl_show_os_version = ($simple==0) && in_array( 'os_version', $t_fields );

	$tpl_show_versions = ($simple==0) && version_should_show_product_version( $t_project_id );
	...
	$tpl_show_due_date = ($simple==0) && in_array( 'due_date', $t_fields ) && access_has_project_level( config_get( 'due_date_update_threshold' ), helper_get_current_project(), auth_get_current_user_id() );
	$tpl_show_attachments = in_array( 'attachments', $t_fields ) && file_allow_bug_upload();
	$tpl_show_view_state = ($simple==0) && in_array( 'view_state', $t_fields ) && access_has_project_level( config_get( 'set_view_status_threshold' ) );
...
<?php
	...
	foreach( $t_related_custom_field_ids as $t_id ) {
		...
		if( ( $t_def['display_report'] || $t_def['require_report']) && ($simple==0) && custom_field_has_write_access_to_project( $t_id, $t_project_id ) ) {
