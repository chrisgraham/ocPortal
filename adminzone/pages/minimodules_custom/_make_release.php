...
$releases_category_id=$GLOBALS['SITE_DB']->query_select_value_if_there('download_categories c JOIN '.get_table_prefix().'translate t ON t.id=c.category','c.id',array('parent_id'=>db_get_first_id(),'text_original'=>'Releases'));
...
$release_category_id=$GLOBALS['SITE_DB']->query_select_value_if_there('download_categories c JOIN '.get_table_prefix().'translate t ON t.id=c.category','c.id',array('parent_id'=>$releases_category_id,'text_original'=>'Version '.strval(intval($version_dotted))));
...
$installatron_category_id=$GLOBALS['SITE_DB']->query_select_value_if_there('download_categories c JOIN '.get_table_prefix().'translate t ON t.id=c.category','c.id',array('parent_id'=>$releases_category_id,'text_original'=>'Installatron integration'));
...
$microsoft_category_id=$GLOBALS['SITE_DB']->query_select_value_if_there('download_categories c JOIN '.get_table_prefix().'translate t ON t.id=c.category','c.id',array('parent_id'=>$releases_category_id,'text_original'=>'Microsoft integration'));
...
foreach ($all_downloads_to_add as $i=>$d)
{
	...
	$download_id=$GLOBALS['SITE_DB']->query_select_value_if_there('download_downloads d JOIN '.get_table_prefix().'translate t ON t.id=d.name','d.id',array('category_id'=>$category_id,'text_original'=>$name));
...
{
	$last_version_str=$GLOBALS['SITE_DB']->query_select_value_if_there('download_downloads d JOIN '.get_table_prefix().'translate t ON d.comments=t.id','t.id',array('text_original'=>'This is the latest version.'),' AND d.id<>'.strval($all_downloads_to_add[0]['download_id']));
	if (!is_null($last_version_str))
	{
		$last_version_id=$GLOBALS['SITE_DB']->query_select_value_if_there('download_downloads d JOIN '.get_table_prefix().'translate t ON d.comments=t.id','d.id',array('text_original'=>'This is the latest version.'),' AND d.id<>'.strval($all_downloads_to_add[0]['download_id']));
...
$news_category=$GLOBALS['SITE_DB']->query_select_value_if_there('news_categories c JOIN '.get_table_prefix().'translate t ON c.nc_title=t.id','c.id',array('text_original'=>'New releases'));
...
$news_id=$GLOBALS['SITE_DB']->query_select_value_if_there('news d JOIN '.get_table_prefix().'translate t ON t.id=d.title','d.id',array('news_category'=>$news_category,'text_original'=>$news_title));
if (is_null($news_id))
{
