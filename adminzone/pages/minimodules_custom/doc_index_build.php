...
foreach ($_addons as $addon=>$place)
{
	if ($place=='sources')
	{
		...
		if (count($tutorials)==0) warn_exit('Missing tutorial for: '.$addon);
		...
		{
			if (in_array($ss[0],array('export','permission'))) continue;
			...
			if (in_array($stemmed_addon,$_ss))
			{
				...
				if ($test!==false) unset($_synonyms[$test]);
		...
		$addons[$addon]=array(
			...
			'tutorials'=>$tutorials,
			'synonyms'=>$_synonyms,
			'tracker_url'=>'http://ocportal.com/tracker/search.php?project_id=1&category='.urlencode($addon).'&status_id=10',
		);
	}
}
