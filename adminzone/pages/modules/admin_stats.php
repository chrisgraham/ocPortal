...
class Module_admin_stats
{
	...
	function overview()
	{
		...
		$rows=$GLOBALS['SITE_DB']->query('SELECT date_and_time FROM '.get_table_prefix().'stats WHERE ('.$where.') ORDER BY '.$sortable.' '.$sort_order,$reasonable_max);
		if (count($rows)<1)
		{
	...
	function page_x_share($page,$type,$graph_title,$graph_description,$list_title)
	{
		...
		foreach ($rows as $value)
		{
			...
			if (!array_key_exists($value[$type],$data1)) $data1[$value[$type]]=0;
			$data1[$value[$type]]++;
		}
		if ($sortable=='views')
		{
