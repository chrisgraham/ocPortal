				...
				foreach ($under as $filename=>$type)
				{
					if (substr(strtolower($filename),-4)=='.txt')
					{
						...
						if (preg_match('#\[block="'.str_replace('#','\#',preg_quote($row['i_menu'])).'"[^\]]* caption="([^"]*)"[^\]]*\]side_stored_menu\[/block\]#',$contents,$matches)!=0)
						{
							...
							$menu_name=do_lang_tempcode('MENU_FULL_DETAILS',escape_html($menu_name),make_string_tempcode(escape_html($matches[1])),make_string_tempcode(escape_html($zone_title)));
							$found=true;
							break 2;
						}
					}
			...
			foreach ($under as $filename=>$type)
			{
				if (substr(strtolower($filename),-4)=='.txt')
				{
					...
					for ($i=0;$i<$num_matches;$i++)
					{
						...
						$menu_name=do_lang_tempcode('MENU_FULL_DETAILS',escape_html($menu_name),make_string_tempcode(escape_html($matches[2][$i])),make_string_tempcode(escape_html($zone_title)));
