...
class Module_admin_newsletter extends standard_aed_module
{
	...
	function misc()
	{
		...
		return do_next_manager(get_page_title('MANAGE_NEWSLETTER'),comcode_lang_string('DOC_NEWSLETTER'),
					array_merge(array(
	...
	function automatic_whats_new()
	{
		...
		$title=get_page_title('NEW_CONTENT');
					...
					if ($num_matches<500) /*reasonable limit*/
					{
						for ($i=0;$i<$num_matches;$i++)
						{
							...
							if (!$hook_content->is_empty())
							{
								...
								$content.=$_title.': '.trim($decoded).' ['.$hook.'/'.$matches[1][$i]."]\n";
							}
						}
						$done=true;
					}
		...
		return $this->send_gui($completed->evaluate($lang));
	}
		...
		foreach ($levels as $level)
		{
			...
			do
			{
				...
				foreach ($rows as $r)
				{
					...
					$unsub=array_key_exists('the_password',$r)?md5('xunsub'.$r['the_password']):'';
