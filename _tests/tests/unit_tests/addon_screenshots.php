 ...
 */
class addon_screenshots_test_set extends ocp_test_case
{
	function testNoUnmatchedScreenshots()
	{
	...
	function testNoMissingScreenshots()
	{
		...
		foreach ($hooks as $hook=>$place)
		{
			if ($place=='sources_custom')
			{
				...
				if ($ob->get_category()=='Development') continue;
				...
				{
					if (is_file(get_file_base().'/data_custom/addon_screenshots/'.$hook.'.'.$ext)) $exists=true;
				}
