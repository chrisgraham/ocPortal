	...
	while (!is_null($next))
	{
		switch ($next)
		{
			case 'FUNCTION':
				...
				{
					if ($_['name']==$_function['name']) log_warning('Duplicated function \''.$_function['name'].'\'');
				}
			...
			case 'INTERFACE':
				...
				$next=pparse__parser_peek();
				if ($next=='IMPLEMENTS') log_warning('Interfaces cannot implement each other, they can only extend each other');
				if ($next=='EXTENDS')
				{
...
// Choice{"FUNCTION" | variable "DEC" | variable "INC" | target assignment_operator expression | function "BRACKET_OPEN" comma_expressions "BRACKET_CLOSE" | "IF" expression command if_rest? | "SWITCH" expression "CURLY_OPEN" cases "CURLY_CLOSE" | "FOREACH" "BRACKET_OPEN" expression "AS" _foreach "BRACKET_CLOSE" command | "FOR" "BRACKET_OPEN" command expression command "BRACKET_CLOSE" command | "DO" command "WHILE" "BRACKET_OPEN" expression "BRACKET_CLOSE" | "WHILE" "BRACKET_OPEN" expression "BRACKET_CLOSE" command | "RETURN" | "CONTINUE" | "BREAK" | "BREAK" expression | "CONTINUE" expression | "RETURN" expression | "GLOBAL" comma_variables | "ECHO" expression}
	...
	$supress_error=($next[0]=='SUPRESS_ERROR');
	if ($supress_error)
	{
		pparse__parser_next();
		$next=pparse__parser_peek(true);
	...
	switch ($next[0])
	{
		...
		case 'DEC':
			pparse__parser_next();
			$variable=_parse_variable($supress_error);
		...
		case 'IDENTIFIER': // Direct function call
			...
			} else
			{
				...
				$command=array('CALL_DIRECT',$identifier,$parameters,$supress_error,$GLOBALS['i']);
			}
			while (pparse__parser_peek()=='OBJECT_OPERATOR')
			{
				pparse__parser_next();
				$command=_parse_call_chain($command,$supress_error);
			}
		...
		case 'FOREACH':
			...
			{
				pparse__parser_next();
				$_foreach=array($variable,_parse_variable($supress_error));
			} else
			{
				$_foreach=$variable;
			}
...
function _parse_call_chain($command=array(),$supress_error=false)
{
	...
	switch (pparse__parser_peek())
	{
		case 'BRACKET_OPEN':
			...
			$command=array('CALL_METHOD',$expression,$args,$supress_error,$GLOBALS['i']);
			break;
		default:
...
// Choice{"VAR" "IDENTIFIER" "EQUAL" literal "COMMAND_TERMINATE" | "VAR" "IDENTIFIER" "COMMAND_TERMINATE" | function_dec}*
	...
	while (($next=='CONST') || ($next=='VAR') || ($next=='FUNCTION') || ($next=='PUBLIC') || ($next=='PRIVATE') || ($next=='PROTECTED') || ($next=='ABSTRACT'))
	{
		switch ($next)
		{
			case 'PRIVATE':
			case 'PROTECTED':
				if ($is_interface) log_warning('Interfaces cannot contain anything protected or private');
			case 'PUBLIC':
...
function _parse_class_dec($modifiers=array())
{
	...
	if ($next=='IMPLEMENTS')
	{
		...
		if (!isset($class['interfaces'])) $class['interfaces']=array();
...
// Choice{"BOOLEAN_NOT expression | SUBTRACT expression | literal | variable | variable "BRACKET_OPEN" comma_parameters "BRACKET_CLOSE" | "IDENTIFIER" | "IDENTIFIER" "BRACKET_OPEN" comma_parameters "BRACKET_CLOSE" | "NEW" "IDENTIFIER" "BRACKET_OPEN" comma_expressions "BRACKET_CLOSE" | "NEW" "IDENTIFIER" | "CLONE" expression | "ARRAY" "BRACKET_OPEN" create_array "BRACKET_CLOSE" | "BRACKET_OPEN" expression "BRACKET_CLOSE" | "BRACKET_OPEN" assignment "BRACKET_CLOSE"}
	...
	$supress_error=($next=='SUPRESS_ERROR');
	if ($supress_error)
	{
		pparse__parser_next();
		$next=pparse__parser_peek();
	...
	switch ($next)
	{
		...
		case 'IDENTIFIER':
			...
			elseif ($next_2=='BRACKET_OPEN') // Is it an inline direct function call
			{
				...
				$parameters=_parse_comma_expressions();
				pparse__parser_expect('BRACKET_CLOSE');
				$expression=array('CALL_DIRECT',$next[1],$parameters,$supress_error,$GLOBALS['i']);
		...
		case 'BRACKET_OPEN':
			...
			if ($next_3=='EQUAL')
			{
				$target=_parse_variable($supress_error);
				pparse__parser_expect('EQUAL');
		...
		default: // By elimination: Must be a variable or a call chained to a variable
			$expression=_parse_variable($supress_error,true);
	}
	if (in_array($expression[0],array('CALL_DIRECT','CALL_INDIRECT','CALL_METHOD'),true))
	{
		...
		{
			pparse__parser_next();
			$expression=_parse_call_chain($expression,$supress_error);
		}
	}
	return $expression;
}

function _parse_variable($supress_error,$can_be_dangling_method_call_instead=false)
{
// Choice{"variable" | "variable" "OBJECT_OPERATOR" variable | "variable" "OBJECT_OPERATOR" "IDENTIFIER" | "variable" "EXTRACT_OPEN" expression "EXTRACT_CLOSE"}
	...
	$next=pparse__parser_peek(true);
	$supress_error=$supress_error || ($next[0]=='SUPRESS_ERROR');
	if ($next[0]=='SUPRESS_ERROR')
	{
		pparse__parser_next();
		$next=pparse__parser_peek(true);
	}
	if ($variable[0]!='variable') parser_error('Expected variable ($) but got '.$variable[0]);
	...
	if ($can_be_dangling_method_call_instead)
	{
		if ($next=='BRACKET_OPEN') // Is it an inline indirect function call
		{
			...
			return array('CALL_INDIRECT',$variable,$parameters,$supress_error,$GLOBALS['i']);
		}
	}

	$variable_chain=_parse_variable_dereferencing_chain_segment($supress_error,$can_be_dangling_method_call_instead);
...
{
	if ($actual_expression===NULL) return false;
	if ($actual_expression==array()) return false;
	if ($actual_expression[0]=='CALL_METHOD') return true;
	return _parse_is_non_pure_variable($actual_expression[2]);
}

function _parse_variable_dereferencing_chain_segment($supress_error)
{
	$next=pparse__parser_peek();
	switch ($next)
	{
		case 'OBJECT_OPERATOR':
			...
			if ((($next_3=='EXTRACT_OPEN') && ($next_4!='EXTRACT_CLOSE')) || ($next_3=='OBJECT_OPERATOR') || ($next_3=='BRACKET_OPEN')) $tunnel=_parse_variable_dereferencing_chain_segment($supress_error);
		...
		case 'BRACKET_OPEN':
			...
			$next_4=pparse__parser_peek_dist(1);
			if ((($next_3=='EXTRACT_OPEN') && ($next_4!='EXTRACT_CLOSE')) || ($next_3=='OBJECT_OPERATOR') || ($next_3=='BRACKET_OPEN')) $tunnel=_parse_variable_dereferencing_chain_segment($supress_error);
			$variable=array('CALL_METHOD',NULL/*will be subbed later for preceding part of chain*/,$args,$supress_error,$GLOBALS['i'],$tunnel);
			break;
...
// Choice{"EQUAL" | "CONCAT_EQUAL" | "DIV_EQUAL" | "MUL_EQUAL" | "MINUS_EQUAL" | "PLUS_EQUAL" | "BOR_EQUAL"}
	...
	if (!in_array($next,array('EQUAL','CONCAT_EQUAL','DIV_EQUAL','MUL_EQUAL','MINUS_EQUAL','PLUS_EQUAL','BOR_EQUAL'))) parser_error('Expected assignment operator but got '.$next);
	return $next;
}
...
// Choice{list | map}?
	...
	$next=pparse__parser_peek();
	if ($next=='BRACKET_CLOSE') return array();
	...
	if ($next=='DOUBLE_ARROW')
	{
		...
		while ($next=='COMMA')
		{
			...
			if (isset($seen[serialize($expression)])) parser_warning('Duplicated key in array creation');
...
// Choice{expression "COMMA" comma_expressions | expression}
	...
	$next=pparse__parser_peek();
	if (($next=='BRACKET_CLOSE') || ($next=='COMMAND_TERMINATE')) return array();
	...
	do
	{
		...
		$next_2=pparse__parser_peek();
		if ($next_2=='COMMA') pparse__parser_next();
	}
	while ($next_2=='COMMA');
...
// Choice{parameter | parameter "COMMA" comma_parameters}?
	...
	$next=pparse__parser_peek();
	if (($next=='BRACKET_CLOSE') || ($next=='COMMAND_TERMINATE')) return $parameters;
