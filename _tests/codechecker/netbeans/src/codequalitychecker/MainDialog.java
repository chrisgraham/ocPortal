...
/**
 * <p>Title: Java Frontend to Web Code Quality Checker</p>
 */
public class MainDialog extends JFrame {
	...
	private void jbInit() throws Exception {
		...
		examineFilesBtn.addActionListener(new
										  Dialog1_examineFilesBtn_actionAdapter(this));
		...
		ClearErrorsBtn.addActionListener(new
										 Dialog1_ClearErrorsBtn_actionAdapter(this));
		...
		scanSignaturesBtn.addActionListener(new
				Dialog1_scanSignaturesBtn_actionAdapter(this));
		...
		EventQueue.invokeLater(new Runnable() { public void run() { scanFilesBtn.doClick(); }});
	}
	...
	public void initiateFileSearch(String type) {
		...
		while (it.hasNext())
		{
			SearchFile next=(SearchFile)it.next();
	...
	public ArrayList initiateFileSearch(String type, String path, String rec_subpath,
		...
		if (thefiles==null) return finalFiles;
	...
	public void countBtn_actionPerformed(ActionEvent e) {
		...
		for (i = 0; i < listModel.getSize(); i++) {
			...
			} catch (Exception e2) {} // No contribution to count
		}
	...
	private boolean line_skippable(String line) {
		...
		for (i = 0; i < Main.skipped_errors.size(); i++) {
			...
			if (decomposed.size()<5) continue;

			same_0 = ((String) decomposed.get(0)).equals((String)
					skip_decomposition.get(0));
			same_1 = ((String) decomposed.get(1)).equals((String)
					skip_decomposition.get(1));
			same_3 = ((String) decomposed.get(3)).equals((String)
					skip_decomposition.get(3));
			same_4 = ((String) decomposed.get(4)).equals((String)
					skip_decomposition.get(4));
			try{
	...
	public void ForgetErrorBtn_actionPerformed(ActionEvent e) {
		...
		try {
			...
			for (i=0;i<Main.skipped_errors.size();i++)
			{
				out.println((String)Main.skipped_errors.elementAt(i));
			}
			out.close();
		} catch (Exception e2) {} // No skip-saving then
	}
}
class Dialog1_errors_actionAdapterKey implements KeyListener {
	private MainDialog adaptee;
...
class FSTransfer extends TransferHandler {
	...
	private MainDialog adaptee = null;
	...
	public boolean importData(JComponent comp, Transferable t) {
		...
		try {
			java.util.List data = (java.util.List) t.getTransferData(DataFlavor.
					javaFileListFlavor);
