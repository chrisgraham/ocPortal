...
<body class="re_body" id="main_website"{$?,{$VALUE_OPTION,html5}, itemscope="itemscope" itemtype="http://schema.org/WebPage"}>
	<div class="main-container">
		{+START,IF,{SHOW_TOP}}
			<div class="logo-part"><a href="{$PAGE_LINK*,:start}"><img src="{$IMG*,logo}" width="188" height="45" alt="{$SITE_NAME*}" /></a></div>
			<div class="menu1-part">
				{$BLOCK,block=side_stored_menu,param=zone_menu,type=zone}
			</div>

			<div class="menu2-part">
				{+START,IF,{$MATCH_KEY_MATCH,:_WILD,site:_WILD}}
					{$BLOCK,block=side_stored_menu,param=main_website,type=tree}
				{+END}

				<div class="search-part">{$BLOCK,block=main_search}</div>
			</div>
		{+END}
