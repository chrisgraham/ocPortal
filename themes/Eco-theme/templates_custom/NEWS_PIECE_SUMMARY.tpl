<div class="float_surrounder news_piece_summary">
	...
	<div class="page_subtitle_tagline">
		{!POSTED_TIME,{DATE*}}{+START,IF,{BLOG}}, {!BY_SIMPLE,<a href="{$MEMBER_PROFILE_LINK*,{SUBMITTER}}">{$USERNAME*,{SUBMITTER}}</a>}{+END}
	</div>
...
<p class="news_goto">
	...
	<a title="{!READ_MORE}: #{ID*}" href="{FULL_URL*}">{!READ_MORE}</a>{+START,IF,{$NOT,{$MATCH_KEY_MATCH,forum:topicview}}} {+START,IF_PASSED,COMMENT_COUNT} ({$COMMENT_COUNT,news,{ID}}){+END}{+END}
</p>
