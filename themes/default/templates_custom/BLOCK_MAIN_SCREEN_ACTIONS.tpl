<{$?,{$VALUE_OPTION,html5},aside,div} class="medborder_box medborder" id="screen_actions_outer"><div id="screen_actions">
	...
	<div class="print"><a title="{!PRINT_THIS_SCREEN}" rel="print" target="_blank" title="{!PRINT_THIS_SCREEN}: {!LINK_NEW_WINDOW}" href="{PRINT_URL*}"></a></div>
	<div class="recommend"><a title="{!OCP_RECOMMEND}" target="_blank" title="{!OCP_RECOMMEND}: {!LINK_NEW_WINDOW}" href="{RECOMMEND_URL*}"></a></div>
	{+START,IF,{$BROWSER_MATCHES,ie}}
		<div class="favorites"><a title="{!ADD_TO_FAVORITES}" href="#" onclick="window.external.AddFavorite(window.location.href,document.title); return false;"></a></div>
	{+END}
	{$,<div class="facebook"><a title="{!ADD_TO_FACEBOOK}" target="_blank" title="{!ADD_TO_FACEBOOK}: {!LINK_NEW_WINDOW}" href="http://www.facebook.com/sharer.php?u={EASY_SELF_URL*}"></a></div>}
