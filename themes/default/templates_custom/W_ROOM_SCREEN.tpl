...
<form method="post" enctype="multipart/form-data" action="{$PAGE_LINK*,_SELF:_SELF:uploading=1}">
	 <div class="wide_table_wrap"><table summary="{!MAP_TABLE}" class="autosized_table results_table wide_table"><tbody>
		  ...
		  <tr>
				<th>{!W_ROOM_TEXT}</th>
				<td><label class="accessibility_hidden" for="text">{!W_ROOM_TEXT}</label><input type="text" size="20" name="text" id="text" value="{ROOM_TEXT*}" /></td>
				<td>{!W_EG_ROOM_TEXT}</td>
		  </tr>
		  <tr>
				<th>{!W_ENTRANCE_QUESTION}</th>
				<td><label class="accessibility_hidden" for="password_question">{!W_ENTRANCE_QUESTION}</label><input type="text" size="20" name="password_question" id="password_question" value="{PASSWORD_QUESTION*}" /></td>
				<td>{!W_EG_ENTRANCE_QUESTION}</td>
		  </tr>
		  <tr>
				<th>{!W_ENTRANCE_ANSWER}</th>
				<td><label class="accessibility_hidden" for="password_answer">{!W_ENTRANCE_ANSWER}</label><input type="text" size="20" name="password_answer" id="password_answer" value="{PASSWORD_ANSWER*}" /></td>
				<td>{!W_EG_ENTRANCE_ANSWER}</td>
		  </tr>
		  <tr>
				<th>{!W_ENTRANCE_FAIL_MESSAGE}</th>
				<td><label class="accessibility_hidden" for="password_fail_message">{!W_ENTRANCE_FAIL_MESSAGE}</label><input type="text" size="20" name="password_fail_message" id="password_fail_message" value="{PASSWORD_FAIL_MESSAGE*}" /></td>
				<td>{!W_EG_ENTRANCE_FAIL_MESSAGE}</td>
		  </tr>
		  <tr>
				<th>{!W_REQUIRED_ITEM}</th>
				<td><label class="accessibility_hidden" for="required_item">{!W_REQUIRED_ITEM}</label><input type="text" size="20" name="required_item" id="required_item" value="{REQUIRED_ITEM*}" /></td>
				<td>{!W_EG_REQUIRED_ITEM}</td>
		  </tr>
		  <tr>
				<th>{!W_UP_DIRECTION_LOCKED}</th>
				<td>
					<label class="accessibility_hidden" for="locked_up">{!W_UP_DIRECTION_LOCKED}</label>
				{+START,IF,{LOCKED_UP}}
		  ...
		  <tr>
				...
				<td>
					<label class="accessibility_hidden" for="locked_down">{!W_DOWN_DIRECTION_LOCKED}</label>
		  ...
		  <tr>
				...
				<td>
					<label class="accessibility_hidden" for="locked_right">{!W_RIGHT_DIRECTION_LOCKED}</label>
		  ...
		  <tr>
				...
				<td>
					<label class="accessibility_hidden" for="locked_left">{!W_LEFT_DIRECTION_LOCKED}</label>
