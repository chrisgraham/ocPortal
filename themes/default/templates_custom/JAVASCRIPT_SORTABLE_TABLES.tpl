 ...
 * @version 0.981
 *
 * @history 0.981 2007-03-19 Added SortableTableSort.numeric_comma, additional date parsing formats
 ...
 * Sort Functions
 */
var SortableTableSort = (function(){
  var sort = {};
...
/**
 * The main SortableTable namespace
 */
var SortableTable = (function(){
  ...
  function getParent(o,a,b) {
    if (o!=null && o.nodeName) {
      if (o.nodeName.toUpperCase()==a || (b && o.nodeName.toUpperCase()==b)) {
        return o;
      }
      while (o=o.parentNode) {
        if (o.nodeName && (o.nodeName.toUpperCase()==a || (b && o.nodeName.toUpperCase()==b))) {
          return o;
        }
      }
    }
  ...
  table.resolve = function(o,args) {
    if (o!=null && o.nodeName && o.nodeName.toUpperCase()!="TABLE") {
      var _o = o.getElementsByTagName('table')[0];
      if (_o)
        o = _o;
      else
   ...
   */
  table.processInnerCells = function(t, type, func, arg) {
    if (type == '*') {
      var es = t.getElementsByTagName('*');
      for (var k=0,L3=es.length; k<L3; k++) {
        func.call(this, es[k], arg);
      }
    }
   ...
   * Referenced by processInnerCells.
  ...
  table.sort = function(o,args) {
    ...
    args.sorttype = args.sorttype || SortableTableSort['default'];
    ...
    if (typeof(sorttype.convert)=="function") {
      ...
      sorttype=SortableTableSort.alphanumeric;
    }
    ...
    for (var i=0,L=bodies.length; i<L; i++) {
      var tb = bodies[i], tbrows = tb.rows, rows = [], unsortedrows = [];
      ...
      if(!hasClass(tb,table.NoSortClassName)) {
        ...
        if (cRow=tbrows[cRowIndex]){
          ...
          do {
            if (rowCells = cRow.cells) {
		        if (!hasClass(tbrows[cRowIndex],'table-nosort')) {
                ...
                unsortedrows.push(tbrows[cRowIndex]);
              }
            }
        ...
        cRowIndex=0;
        if (cRow=unsortedrows[cRowIndex]){
          do { 
            tb.appendChild(cRow); 
          } while (cRow=unsortedrows[++cRowIndex]);
        }
      }
    }
  ...
  */
  table.filter = function(o,filters,args) {
    ...
    if (!filters) {
      ...
      if (typeof filters.nodeName!='undefined' && filters.nodeName.toUpperCase()=="SELECT" && filters.type=="select-one" && filters.selectedIndex>-1) {
  ...
  */
  table.scrape = function(o,args) {
    ...
    for (var i=0,L=bodies.length; i<L; i++) {
      ...
      for (var j=0,L2=tb.rows.length; j<L2; j++) {
        ...
        if (hasClass(row,'table-nofilter')) continue;
  ...
  table.autosort = function(t,args) {
    t = this.resolve(t,args);
    var tdata = this.tabledata[t.id];
    this.processInnerCells(t, "THEAD", function(c) {
  ...
  table.autopage = function(t,args) {
    ...
    var tdata = this.tabledata[t.id];
    if (tdata.pagesize) {
     this.processInnerCells(t.parentNode, "*", function(c) {
  ...
  table.autofilter = function(t,args) {
    ...
    table.processInnerCells(t, "THEAD", function(cell) {
      if (hasClass(cell,table.FilterableClassName)) {
        ...
        if (colValues.length>0) {
          colValues.sort(SortableTableSort[classValue(cell,table.SortableColumnPrefix) || "default"]);
          /*for (var i=0;i<colValues.length;i++) {	Breaks the filtering code, and not needed
            colValues[i] = colValues[i].replace(/^\s+/,'').replace(/\s+$/,'');
          }*/
