...
<form title="{!PRIMARY_PAGE_FORM}" action="{URL*}" method="post" id="test_form">
	...
	<p class="buttons_group">
		...
		<input class="buttons__yes button_page" type="button" value="{!MARK_ALL_UNDONE}" onclick="window.fauxmodal_confirm('{!MARK_ALL_UNDONE_SURE}',function(answer) { if (answer) mark_all_undone(); });" />
	</p>
</form>
