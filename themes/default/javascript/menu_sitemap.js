...
function _generate_menu_sitemap(target,node,the_level)
{
	...
	if (node.children.length!=0)
	{
		...
		if (the_level<2) // High-levels start expanded
		{
			...
			expand_img.src="{$IMG#,1x/trays/contract}".replace(/^http:/,window.location.protocol);
			expand_img.srcset="{$IMG#,2x/trays/contract}".replace(/^http:/,window.location.protocol)+' 2x';
		} else
		{
			expand_img.alt="{!EXPAND^#}";
			expand_img.src="{$IMG#,1x/trays/expand}".replace(/^http:/,window.location.protocol);
			expand_img.srcset="{$IMG#,2x/trays/expand}".replace(/^http:/,window.location.protocol)+' 2x';
