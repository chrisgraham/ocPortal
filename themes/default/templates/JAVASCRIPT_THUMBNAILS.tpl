...
function nereidFade(fadeElement,destPercentOpacity,periodInMsecs,increment)
{
	...
	} else
	{
		if (fadeElement.style.opacity)
		{
			...
			again=(temp!=destPercentOpacity/100.0);
		} else again=true;
	}
	if (again)
	{
		thumbFadeTimers[fadeElement.faderKey]=window.setTimeout(function() { nereidFade(fadeElement,destPercentOpacity,periodInMsecs,increment); },periodInMsecs);
	}
}
