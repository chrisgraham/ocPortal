...
function reload_preview(id)
{
	...
	do_ajax_request('{$FIND_SCRIPT_NOHTTP;,comcode_convert}?fix_bad_html=1&css=1&javascript=1&is_semihtml='+(isWYSIWYGField(edit_element)?'1':'0')+'&panel='+(((id=='panel_left') || (id=='panel_right'))?'1':'0')+keep_stub(),reloaded_preview,'data='+window.encodeURIComponent(data));
}
