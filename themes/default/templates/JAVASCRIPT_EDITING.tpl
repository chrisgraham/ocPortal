		...
		for (var counter=0;counter<forms[fid].elements.length;counter++)
		{
			...
			if (typeof window.wysiwyg_editors[id]!='undefined')
			{
				...
				} else
				{
					...
					var request=do_ajax_request(url,false,'data='+window.encodeURIComponent(window.wysiwyg_editors[id].getData()));
...
function set_textbox(element,text,html)
{
	if (is_wysiwyg_field(element))
	{
		...
		wysiwyg_editors[element.id].setData(html);

		window.setTimeout(function() {
			find_tags_in_editor(wysiwyg_editors[element.id],element);
		}, 100);
	}
