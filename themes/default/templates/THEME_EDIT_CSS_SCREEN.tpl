...
{+START,IF,{$JS_ON}}
	...
	<form title="{!CSS_EQUATION_HELPER}" action="{$BASE_URL*}/index.php" onsubmit="return false;" method="post">
		<p>
			...
			<input onclick="cancelBubbling(event,this); var result=load_snippet('themewizard_equation&amp;theme={THEME*;}&amp;css_equation='+window.encodeURIComponent(document.getElementById('css_equation').value),false); if (result=='' || result.indexOf('&lt;html')!=-1) window.alert('{!ERROR_OCCURED;}'); else document.getElementById('css_result').value=result; return false;" class="button_pageitem" type="submit" value="{!CALCULATE}" />
