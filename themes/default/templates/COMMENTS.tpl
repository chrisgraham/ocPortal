					...
					<div class="contact-form">
						<ul>
							...
							<li>
								...
								<textarea accesskey="x" class="wide_field" onfocus="if ((this.value=='{POST_WARNING*;}') &amp;&amp; ('{POST_WARNING*;}'!='')) this.value='';" cols="42" rows="11" name="post" id="post">{POST_WARNING*}</textarea>
							</li>
						</ul>
					</div>
				{+END}
						...
						<tbody>
							...
							<tr>
								<th{+START,IF,{$NOT,{$VALUE_OPTION,html5}}} abbr="{!POST_COMMENT}"{+END} class="de_th">
									...
									{+START,IF,{$JS_ON}}
										{+START,IF,{$CONFIG_OPTION,is_on_emoticon_choosers}}
											<br />
											{+START,BOX,,,med}
												<div class="comments_emoticons">
													{EM}

													{+START,IF,{$OCF}}
														<p><a tabindex="6" href="#" onclick="window.open(maintain_theme_in_link('{$FIND_SCRIPT*,emoticons}?field_name=post{$KEEP*;}'),'site_emoticon_chooser','width=180,height=500,status=no,resizable=yes,scrollbars=no'); return false;">{!EMOTICONS_POPUP}</a></p>
													{+END}
												</div>
											{+END}
										{+END}
								...
								<td>
									...
									<div class="constrain_field">
										<textarea onkeyup="manageScrollHeight(this);" accesskey="x" class="wide_field" onfocus="if ((this.value=='{POST_WARNING*;}') &amp;&amp; ('{POST_WARNING*;}'!='')) this.value='';" tabindex="3" cols="42" rows="11" name="post" id="post">{POST_WARNING*}</textarea>
									</div>
