			...
			<div class="float_surrounder chat_im_convos_inner">
				...
				<div class="chat_lobby_buddies">
					...
					{+START,IF_NON_EMPTY,{URL_ADD_BUDDY}}
						...
						<form onsubmit="if (!checkFieldForBlankness(this.elements['buddy_username'],event)) return false;" title="{!ADD}: {!BUDDY_LIST}" method="post" action="{URL_ADD_BUDDY*}">
							<label class="accessibility_hidden" for="buddy_username">{!USERNAME}: </label><input size="18" maxlength="80" onkeyup="update_ajax_member_list(this,null,false,event);" type="text" onfocus="if (this.value=='{!USERNAME*;}') { this.value=''; this.style.color='black'; }" onblur="if (this.value=='') { this.value='{!USERNAME*;}'; this.style.color='gray'; }" style="color: gray" alt="{!USERNAME}" value="{!USERNAME}" id="buddy_username" name="buddy_username" />
