			...
			<div class="float_surrounder chat_im_convos_inner">
				...
				<div class="chat_lobby_friends">
					...
					{+START,IF_NON_EMPTY,{URL_ADD_FRIEND}}
						...
						<form onsubmit="if (!check_field_for_blankness(this.elements['friend_username'],event)) return false;" title="{!ADD}: {!FRIEND_LIST}" method="post" action="{URL_ADD_FRIEND*}">
							<label class="accessibility_hidden" for="friend_username">{!USERNAME}: </label><input {+START,IF,{$MOBILE}}autocorrect="off" {+END}autocomplete="off" size="18" maxlength="80" onkeyup="update_ajax_member_list(this,null,false,event);" type="text" onfocus="if (this.value=='{!USERNAME*;}') { this.value=''; this.className='field_input_filled'; }" onblur="if (this.value=='') { this.value='{!USERNAME*;}'; this.className='field_input_non_filled'; }" class="field_input_non_filled" alt="{!USERNAME}" value="{!USERNAME}" id="friend_username" name="friend_username" />
