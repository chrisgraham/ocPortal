	...
	<th{+START,IF,{$NOT,{$VALUE_OPTION,html5}}} abbr="{GROUP_NAME*}"{+END} class="de_th dottedborder_barrier_a_nonrequired">
		...
		{+START,IF,{$OR,{FORCE_PRESETS},{$IS_NON_EMPTY,{OVERRIDES}}}}
			<label for="access_{GROUP_ID*}_presets">
				...
				<select tabindex="{TABINDEX*}" id="access_{GROUP_ID*}_presets" name="access_{GROUP_ID*}_presets" onkeypress="if (enter_pressed(event)) this.onclick(event);" onclick="this.onchange();" onchange="copy_permission_presets('access_{GROUP_ID*;}',this.options[this.selectedIndex].value); cleanup_permission_list('access_{GROUP_ID*;}')">
					{+START,IF,{ALL_GLOBAL}}
