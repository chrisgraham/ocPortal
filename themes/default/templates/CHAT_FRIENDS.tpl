	...
	<div class="wide_table_wrap"><table summary="{!COLUMNED_TABLE}" class="results_table wide_table autosized_table">
		...
		<tbody>
			{+START,LOOP,FRIENDS}
				<tr>
					<td>
						<img {+START,IF,{SIMPLER}}title="{ONLINE_TEXT*}" {+END}id="friend_img_{MEMBER_ID*}" alt="" src="{$IMG*,menu_items/community_navigation/members}" />
					</td>
					...
					<td>
						...
						{+START,IF,{CAN_IM}}
							<a rel="friend" title="{USERNAME*}: {!START_IM}" href="{$PAGE_LINK*,_SEARCH:chat:enter_im={MEMBER_ID}}" onclick="if (typeof window.start_im=='undefined') return true; start_im('{MEMBER_ID*}',true{$,{$?,{SIMPLER},true,false}}); return false;">{USERNAME*}</a>
						{+END}
						{+START,IF,{$NOT,{CAN_IM}}}
							{USERNAME*}
						{+END}
