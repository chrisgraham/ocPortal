						...
						<td class="form_table_field_input" colspan="2">
							{+START,IF,{$NOT,{$VALUE_OPTION,disable_boolean_search}}}
								...
								{+END}
									{+START,IF,{OLD_MYSQL}}
										<p>
											<label for="conjunctive_operator_and"><input type="radio" {+START,IF,{AND}}checked="checked" {+END}id="conjunctive_operator_and" name="conjunctive_operator" value="AND" />{!SEARCH_AND}</label>
											<label for="conjunctive_operator_or"><input type="radio" {+START,IF,{$NOT,{AND}}}checked="checked" {+END}id="conjunctive_operator_or" name="conjunctive_operator" value="OR" />{!SEARCH_OR}</label>
										</p>
									{+END}
									<p>{!BOOLEAN_HELP}</p>
								{+START,IF,{$NOT,{OLD_MYSQL}}}
