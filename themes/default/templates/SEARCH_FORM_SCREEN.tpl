...
{+START,IF_PASSED,RESULTS}
	...
	{+START,IF_NON_EMPTY,{RESULTS}}
		<div class="float_surrounder"{$?,{$VALUE_OPTION,html5}, itemscope="itemscope" itemtype="http://schema.org/SearchResultsPage"}>
			{RESULTS}
		</div>
				...
				<tr>
					...
					<td class="dottedborder_barrier" colspan="2">
						...
						<div class="constrain_field">
							<input maxlength="255" onkeyup="update_ajax_search_list(this,event);" onkeypress="if (enter_pressed(event)) this.form.submit();" class="wide_field" type="{+START,IF,{$VALUE_OPTION,html5}}search{+END}{+START,IF,{$NOT,{$VALUE_OPTION,html5}}}text{+END}" size="48" id="search_content" name="content" value="{+START,IF_PASSED,CONTENT}{CONTENT*}{+END}" />
						</div>
...
<form title="{!SEARCH}: Google" alass="google_search_box" method="get" action="http://www.google.com/search">
	<div class="lightborder inline_block">
