			...
			{+START,IF,{$NOT,{$MOBILE}}}{+START,IF,{$OR,{$CONFIG_OPTION,is_on_emoticon_choosers},{$AND,{$OCF},{$JS_ON}}}}
				<div class="emoticon_chooser lightborder"><div class="float_surrounder">
					{+START,IF,{$AND,{$OCF},{$JS_ON}}}
						<div class="right">
							[ <a target="_blank" href="{$FIND_SCRIPT*,emoticons}?field_name={NAME*}{$KEEP*;,0,1}" onclick="window.open(maintain_theme_in_link('{$FIND_SCRIPT*,emoticons}?field_name={NAME*}{$KEEP*;,0,1}'),'site_emoticon_chooser','width=180,height=500,status=no,resizable=yes,scrollbars=no'); return false;" class="posting_form_sup_link" title="{!EMOTICONS_POPUP} {!LINK_NEW_WINDOW}">{!VIEW_ARCHIVE}</a> ]
