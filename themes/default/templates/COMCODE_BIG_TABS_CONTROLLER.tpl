...
<div class="comcode_big_tab_controller" role="tablist">
	{+START,LOOP,TABS}
		<div class="{$?,{$EQ,{_loop_key},0},big_tab_active big_tab_first,big_tab_inactive}" id="{PASS_ID|*}_{$GET%,big_tab_sets}_btgoto_{_loop_var|*}">
			<a aria-controls="{PASS_ID|*}_{$GET%,big_tab_sets}_section_{_loop_var|*}" role="tab" href="#" onclick="return flip_page('{_loop_var|;*}','{PASS_ID|;*}_{$GET%,big_tab_sets}',a{PASS_ID|*}_{$GET%,big_tab_sets}_big_tab);">{_loop_var*}</a>
		</div>
	{+END}
</div>
