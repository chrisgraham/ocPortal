	...
	<div class="box box_arrow box__block_top_notifications_web"><span></span><div class="box_inner">
		<div id="web_notifications_spot">
			{+START,IF_EMPTY,{NOTIFICATIONS}}
				<p class="nothing_here">{!NO_NOTIFICATIONS}</p>
			{+END}
...
{+START,IF,{$NOT,{$CONFIG_OPTION,pt_notifications_as_web}}}{+START,IF,{$OCF}}
	...
	<a title="{!ocf:PRIVATE_TOPICS}" id="pts_button" class="no_tooltip count_{NUM_UNREAD_PTS%}" onclick="return toggle_pts(event);" href="{$PAGE_LINK*,_SEARCH:members:view#tab__pts}"><span>{NUM_UNREAD_PTS*}</span></a>
{+END}{+END}
