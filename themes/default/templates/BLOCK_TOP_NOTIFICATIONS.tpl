...
<div class="top_notifications">
	...
	<a title="{!notifications:NOTIFICATIONS}" id="web_notifications_button" class="count_{NUM_UNREAD_WEB_NOTIFICATIONS%}" onclick="return toggle_web_notifications(event);" href="{$PAGE_LINK*,_SEARCH:notifications:browse}"><span>{NUM_UNREAD_WEB_NOTIFICATIONS*}</span></a>
	...
	{+START,IF,{$NOT,{$CONFIG_OPTION,pt_notifications_as_web}}}{+START,IF,{$OCF}}
		<div class="global_footer_button_ref_point" id="pts_rel" style="display: none">
			<div class="box box_arrow box__block_top_notifications_pts"><span></span><div class="box_inner">
				...
				<ul class="associated_links_block_group horizontal_links">
					<li><a href="{$PAGE_LINK*,_SEARCH:members:view#tab__pts}">{!ocf:PRIVATE_TOPICS_INBOX}</a></li>
					<li><a href="{$PAGE_LINK*,_SEARCH:topics:new_pt}">{!ocf:NEW_PRIVATE_TOPIC}</a></li>
				</ul>
			</div></div>
		</div>
		<a title="{!ocf:PRIVATE_TOPICS}" id="pts_button" class="count_{NUM_UNREAD_PTS%}" onclick="return toggle_pts(event);" href="{$PAGE_LINK*,_SEARCH:members:view#tab__pts}"><span>{NUM_UNREAD_PTS*}</span></a>
	{+END}{+END}
