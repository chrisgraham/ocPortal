...
tree_list.prototype.render_tree=function(xml,html,element)
{
	...
	for (i=0;i<xml.childNodes.length;i++)
	{
		...
		if (node.nodeName.toLowerCase()=='category')
		{
			...
			setInnerHTML(node_self,'<input style="width: 11px; height: 11px"'+(this.tabindex?' tabindex="'+this.tabindex+'"':'')+' type="image" alt="'+((!initially_expanded)?'{!EXPAND^;}':'{!CONTRACT^;}')+': '+escaped_title+'" title="'+((!initially_expanded)?'{!EXPAND^;}':'{!CONTRACT^;}')+'" id="'+this.name+'texp_c_'+node.getAttribute('id')+'" src="'+((!initially_expanded)?'{$IMG*,treefield/plus}':'{$IMG*,treefield/minus}').replace(/^http:/,window.location.protocol)+'" /> <img style="width: 14px; height: 14px" class="treelist_cat_icon" alt="{!CATEGORY^;}" src="'+'{$IMG*,treefield/category}'.replace(/^http:/,window.location.protocol)+'" /> <label id="'+this.name+'tsel_c_'+node.getAttribute('id')+'" for="'+this.name+'tsel_r_'+node.getAttribute('id')+'" onmouseout="if (typeof window.deactivateTooltip!=\'undefined\') deactivateTooltip(this,event);" onmousemove="if (typeof window.activateTooltip!=\'undefined\') repositionTooltip(this,event);" onmouseover="if (typeof window.activateTooltip!=\'undefined\') activateTooltip(this,event,'+(node.getAttribute('description_html')?'':'escape_html')+'(this.childNodes[0].title),\'auto\');" style="background-color: white; color: black; cursor: pointer; color: '+colour+'"><input tabindex="'+this.tabindex+'" id="'+this.name+'tsel_r_'+node.getAttribute('id')+'" style="position: absolute; left: -10000px" type="radio" name="_'+this.name+'" value="1" title="'+description_in_use+'" />'+escaped_title+'</label> <span id="'+this.name+'extra_'+node.getAttribute('id')+'">'+extra+'</span><br />');
