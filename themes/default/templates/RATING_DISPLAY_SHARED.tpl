...
{+START,IF,{$OR,{LIKES},{$IS_EMPTY,{$TRIM,{RATING_FORM}}}}}
	...
	{$SET,rating_loop,0}
	{+START,WHILE,{$LT,{$GET,rating_loop},{$ROUND,{$DIV_FLOAT,{RATING},2}}}}<img src="{$IMG*,rating}" title="{!HAS_RATING,{$ROUND,{$DIV_FLOAT,{RATING},2}}}" alt="{!HAS_RATING,{$ROUND,{$DIV_FLOAT,{RATING},2}}}" />{$INC,rating_loop}{+END}
