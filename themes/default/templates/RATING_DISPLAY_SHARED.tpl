...
{+START,IF,{$OR,{LIKES},{$IS_EMPTY,{$TRIM,{RATING_FORM}}}}}
	...
	{+START,WHILE,{$LT,{$GET,rating_loop},{$GET,rating_stars}}}<img src="{$IMG*,rating}" {$?,{$EQ,{$GET,rating_loop},0},alt="{!HAS_RATING,{$GET,rating_stars}}" title="{!HAS_RATING,{$GET,rating_stars}}",alt=""} />{$INC,rating_loop}{+END}
