function match_calendar_from_to(stub)
{
	if (stub.match(/\_(from|start)$/)) // If we are a 'from' date and we have a matching 'to' date that is earlier than it, copy the 'from' date across to the 'to' date to make it a bit quicker to set an accurate 'to' date
	{
		var year = document.getElementById(stub + '_year');
		var month=document.getElementById(stub + '_month');
		...
		{
			to_day.selectedIndex=day.selectedIndex;
			to_month.selectedIndex=month.selectedIndex;
			to_year.selectedIndex=year.selectedIndex;
		}
	}
}
...
if (typeof window.YAHOO != 'undefined')
{
	function initialise_date_field(stub, calob, linkob, unlimited, mindate, maxdate)
	{
		...
		} else
		{
			...
			window[calob].onSelect = function()
			{
				...
				{
					year.selectedIndex = new_year_index;
				}
				var month=document.getElementById(stub + '_month');
	...
	YAHOO.widget.DateMath = new function() {
		...
		this.getWeekNumber = function(date, calendarYear) {
			if (!calendarYear) {
	...
	YAHOO.widget.Calendar_Core.prototype.setupConfig = function() {
		...
		this.Config.Options = { MULTI_SELECT: false,SHOW_WEEKDAYS: true,START_WEEKDAY: 1,SHOW_WEEK_HEADER: false,SHOW_WEEK_FOOTER: false,HIDE_BLANK_WEEKS: false,NAV_ARROW_LEFT: "http://localhost/git/themes/default/images/date_chooser/callt.gif".replace(/^http:/, window.location.protocol),NAV_ARROW_RIGHT: "http://localhost/git/themes/default/images/date_chooser/calrt.gif".replace(/^http:/, window.location.protocol) };
	...
	YAHOO.widget.Calendar2up.prototype.renderHeader = function() {
		...
		if (this.outerContainer.style.position == "absolute") 
		{
			...
			imgClose.src = "http://localhost/git/themes/default/images/date_chooser/calx.gif".replace(/^http:/, window.location.protocol);
