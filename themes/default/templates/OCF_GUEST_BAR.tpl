<div class="standardbox_wrap_classic ocf_information_bar_outer">
	<div class="standardbox_classic">
		<h2 class="standardbox_title_classic">{!_LOGIN}{+START,IF,{$HAS_ACTUAL_PAGE_ACCESS,search}} / {!SEARCH}{+END}</h2>
			...
			<div class="ocf_guest_column ocf_guest_column_a">
				<form title="{!_LOGIN}" onsubmit="if (checkFieldForBlankness(this.elements['login_username'],event)) { disable_button_just_clicked(this); return true; } return false;" action="{LOGIN_URL*}" method="post" class="autocomplete inline">
					<div>
						...
						<input maxlength="80" size="15" type="text" onfocus="if (this.value=='{!USERNAME;}'){ this.value=''; password.value=''; }" value="{!USERNAME}" id="login_username" name="login_username" />
						<input maxlength="255" size="15" type="password" value="password" name="password" id="s_password" />
						<label for="remember">{!REMEMBER_ME}</label> <input {+START,IF,{$CONFIG_OPTION,remember_me_by_default}}checked="checked" {+END}{+START,IF,{$NOT,{$CONFIG_OPTION,remember_me_by_default}}}onclick="if (this.checked) if (!window.confirm('{!REMEMBER_ME_COOKIE;}')) this.checked=false;" {+END}type="checkbox" value="1" id="remember" name="remember" />
