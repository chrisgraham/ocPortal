<div class="installer_main_min">
	<p>
		{!INSTALL_LOG_BELOW,{PREVIOUS_STEP*}}:
	</p>

	<div><div class="install_log_table">
		<p class="lonely_label">{!INSTALL_LOG}:</p>
		<ul role="navigation" class="actions_list">
			{LOG}
		</ul>
	</div></div>

	<p>
		{FINAL}
	</p>

	<p>
		{!FINAL_INSTRUCTIONS_B}
	</p>

	<p>
		{!FINAL_INSTRUCTIONS_C}
	</p>

	<nav role="navigation" class="installer_completed_calltoaction">
		<ul class="actions_list">
			<li><span class="actions_list_strong"><a href="{$BASE_URL*}/adminzone/index.php?page=admin_setupwizard&amp;type=misc">{!CONFIGURE}</a> ({!RECOMMENDED})</span></li>
			<li><span class="actions_list"><a href="{$BASE_URL*}/index.php">{!GO}</a></span></li>
		</ul>
	</nav>

	<p class="right">
		<em>{!THANKS}</em>
	</p>
</div>
