...
{+START,IF_NON_PASSED_OR_FALSE,FRAMED}
	{+START,IF,{THUMB}}<a {+START,IF_PASSED,CLICK_URL}href="{CLICK_URL*}"{+END}{+START,IF_NON_PASSED,CLICK_URL}rel="lightbox" href="{URL*}"{+END} target="_blank" title="{DESCRIPTION*} {!LINK_NEW_WINDOW}">{+END}<img class="attachment_img"{+START,IF_NON_EMPTY,{WIDTH}} width="{WIDTH*}"{+END}{+START,IF_NON_EMPTY,{HEIGHT}} height="{HEIGHT*}"{+END} src="{THUMB_URL*}"{+START,IF_NON_PASSED_OR_FALSE,WYSIWYG_EDITABLE}{+START,IF,{THUMB}}{+START,IF_PASSED,NUM_DOWNLOADS} alt="{!IMAGE_ATTACHMENT,{$NUMBER_FORMAT*,{NUM_DOWNLOADS}},{CLEAN_FILESIZE*}}"{+END}{+END}{+START,IF,{$NOT,{THUMB}}} title="{DESCRIPTION*}" alt="{DESCRIPTION*}"{+END}{+END}{+START,IF_PASSED_AND_TRUE,WYSIWYG_EDITABLE}{+START,IF,{THUMB}}{+END}{+START,IF,{$NOT,{THUMB}}} title="{DESCRIPTION*}"{+END} alt="{DESCRIPTION*}"{+END} />{+START,IF,{THUMB}}</a>{+END}
{+END}
