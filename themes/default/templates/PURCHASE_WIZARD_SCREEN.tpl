...
{+START,IF_NON_EMPTY,{URL}}
	<p class="purchase_button">
		<input onclick="if (typeof this.form=='undefined') var form=window.form_submitting; else var form=this.form; return do_form_submit(form);" id="proceed_button" class="button_page" accesskey="u" {+START,IF,{$JS_ON}}type="button"{+END}{+START,IF,{$NOT,{$JS_ON}}}type="submit"{+END} value="{!PROCEED}" />
	</p>
</form>
{+END}
