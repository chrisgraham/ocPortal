...
<li class="{$?,{CURRENT},current,non_current} {$?,{$IS_EMPTY,{$GET,img}},has_no_img,has_img}">
	<a class="{+START,IF_EMPTY,{URL}}non_link {+END}{+START,IF,{$GET,HAS_CHILDREN}} drawer{+END}"{+START,IF,{$GET,HAS_CHILDREN}} onclick="return show_mobile_sub_menu(this,'{MENU|*}_pexpand_{$GET;*,RAND}','{URL;*}');"{+END} href="{$?*,{$IS_EMPTY,{URL}},#,{URL}}">
