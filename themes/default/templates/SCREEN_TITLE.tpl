...
<h1 class="screen_title"{+START,IF,{$NOT,{$GET,name_set_elsewhere}}} itemprop="name"{+END} role="banner">
	...
	{+START,IF_PASSED,AWARDS}
		{+START,IF_NON_EMPTY,{AWARDS}}
			...
			{+END}
			<img onmouseover="if (typeof window.activate_tooltip!='undefined') activate_tooltip(this,event,'{$GET^;*,AWARDS_TEXT}','auto',null,null,false,true);" alt="{!AWARD_WINNER}" src="{$IMG*,awarded}" />
		{+END}
	{+END}
