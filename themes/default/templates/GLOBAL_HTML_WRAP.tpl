		...
		{+START,IF,{$SHOW_HEADER}}
			<header class="float_surrounder" itemscope="itemscope" itemtype="http://schema.org/WPHeader">
				...
				<div class="global_navigation">
					...
					{+END}
					{+START,IF,{$EQ,{$ZONE},adminzone,cms}}
				...
				<div class="global_footer_left">
					{+START,SET,FOOTER_BUTTONS}
						...
						{+START,IF,{$ADDON_INSTALLED,bookmarks}}{+START,IF,{$NOT,{$IS_GUEST}}}
							<li><a accesskey="n" id="bookmarks_menu_button" onclick="if (typeof window.load_management_menu=='undefined') { require_javascript('javascript_staff'); window.setTimeout(document.getElementById('bookmarks_menu_button').onclick,10); return false; } else return load_management_menu('bookmarks',true);" href="{$PAGE_LINK*,_SEARCH:bookmarks}&amp;url={$SELF_URL&*}"><img style="position: relative; top: 1px" width="20" height="20" id="bookmarks_menu_img" class="no_theme_img_click" title="{!BOOKMARKS}" alt="{!BOOKMARKS}" src="{$IMG*,footer/bookmarksmenu}" /></a></li>
						{+END}{+END}
					...
					<nav class="global_minilinks" role="navigation">
						<ul class="horizontal_links">
							...
							{+START,IF,{$CONFIG_OPTION,bottom_show_feedback_link}}
								<li><a onclick="return open_link_as_overlay(this);" rel="site_contact" accesskey="9" href="{$PAGE_LINK*,_SEARCH:feedback}{+START,IF,{$NOT,{$IN_STR,{$PAGE_LINK,:feedback},?}}}?{+END}{+START,IF,{$NOT,{$NOT,{$IN_STR,{$PAGE_LINK,:feedback},?}}}}&amp;{+END}redirect={$SELF_URL&*,1}">{!_FEEDBACK}</a></li>
							{+END}
