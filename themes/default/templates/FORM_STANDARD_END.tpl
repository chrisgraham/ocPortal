...
<p class="proceed_button{+START,IF_PASSED,SUBMIT_BUTTON_CLASS} {SUBMIT_BUTTON_CLASS*}{+END}">
	{+START,IF_PASSED,PREVIEW}{+START,IF,{$JS_ON}}{+START,IF,{$CONFIG_OPTION,enable_previews}}
		<input class="button_page" onclick="if (typeof this.form=='undefined') var form=window.form_submitting; else var form=this.form; if (form.onsubmit) { var test=form.onsubmit(); if (!test) return; } if (do_form_preview(form,'{$PREVIEW_URL*;}{$KEEP*;}{+START,IF_PASSED,THEME}&amp;utheme={THEME*}{+END}'+((typeof window.mobile_version_for_preview=='undefined')?'':('&amp;keep_mobile='+(window.mobile_version_for_preview?'1':'0'))){+START,IF_PASSED,SEPARATE_PREVIEW}{+START,IF,{SEPARATE_PREVIEW}},true{+END}{+END})) form.submit(); return;" id="preview_button" accesskey="p" tabindex="{+START,IF_PASSED,TABINDEX}{TABINDEX}{+END}{+START,IF_NON_PASSED,TABINDEX}250{+END}" type="button" value="{!PREVIEW}" />
	{+END}{+END}{+END}
	<input class="button_page" onclick="if (typeof this.form=='undefined') var form=window.form_submitting; else var form=this.form; return do_form_submit(form);" {+START,IF_NON_PASSED,SECONDARY_FORM}id="submit_button" accesskey="u" {+END}tabindex="{+START,IF_PASSED,TABINDEX}{TABINDEX}{+END}{+START,IF_NON_PASSED,TABINDEX}250{+END}" {+START,IF,{$JS_ON}}type="button"{+END}{+START,IF,{$NOT,{$JS_ON}}}type="submit"{+END} value="{SUBMIT_NAME*}" />
</p>
