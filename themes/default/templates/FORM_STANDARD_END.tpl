		...
		<div class="preview_validation_box">
			{+START,BOX,{!PERFORM_CHECKS_ON_PREVIEW},,med}
				{+START,IF,{$CONFIG_OPTION,enable_markup_validation}}
					<div>{!VALIDATION}:
						<input title="{!DESCRIPTION_VALIDATION_ON_PREVIEW_0}" {+START,IF,{$NOT,{$HAS_SPECIFIC_PERMISSION,perform_markup_validation_by_default}}}checked="checked" {+END}type="radio" name="perform_validation" value="0" id="perform_validation_no" /><label for="perform_validation_no">{!NO}</label>
						<input title="{!DESCRIPTION_VALIDATION_ON_PREVIEW_1}" {+START,IF,{$HAS_SPECIFIC_PERMISSION,perform_markup_validation_by_default}}checked="checked" {+END}type="radio" name="perform_validation" value="1" id="perform_validation_yes" /><label for="perform_validation_yes">{!YES}</label>
...
<p class="proceed_button{+START,IF_PASSED,SUBMIT_BUTTON_CLASS} {SUBMIT_BUTTON_CLASS*}{+END}">
	{+START,IF_PASSED,PREVIEW}{+START,IF,{$JS_ON}}{+START,IF,{$CONFIG_OPTION,enable_previews}}
		<input class="button_page" onclick="if (typeof this.form=='undefined') var form=window.form_submitting; else var form=this.form; if (form.onsubmit) { var test=form.onsubmit.call(form,event); if (!test) return; } if (do_form_preview(event,form,'{$PREVIEW_URL*;}{$KEEP*;}{+START,IF_PASSED,THEME}&amp;utheme={THEME*}{+END}'+((typeof window.mobile_version_for_preview=='undefined')?'':('&amp;keep_mobile='+(window.mobile_version_for_preview?'1':'0'))){+START,IF_PASSED,SEPARATE_PREVIEW}{+START,IF,{SEPARATE_PREVIEW}},true{+END}{+END})) form.submit(); return;" id="preview_button" accesskey="p" tabindex="{+START,IF_PASSED,TABINDEX}{TABINDEX}{+END}{+START,IF_NON_PASSED,TABINDEX}250{+END}" type="button" value="{!PREVIEW}" />
	{+END}{+END}{+END}
