...
{+END}{+END}
{+START,IF_NON_EMPTY,{URL}}<a rel="external" title="{CAPTION*}: {!LINK_NEW_WINDOW}" class="link_exempt" target="_blank" href="{$FIND_SCRIPT*,banner}?source={SOURCE*&}&amp;dest={DEST*&}&amp;type=click{$KEEP*,0,1}">{+END}<img width="{WIDTH*}" alt="{CAPTION*}" style="border: 0" title="{CAPTION*}" src="{IMG*}" />{+START,IF_NON_EMPTY,{URL}}</a>{+END}
