	...
	<th {+START,IF,{$NOT,{$MOBILE}}}colspan="2" {+END} class="de_th dottedborder_divider"{+START,IF_PASSED,TITLE} abbr="{$STRIP_TAGS*,{TITLE}}"{+END}>
		{+START,IF_PASSED,TITLE}
			{+START,IF,{$JS_ON}}{+START,IF_NON_PASSED,FORCE_OPEN}
				<a id="fes{TITLE|}" onkeypress="return this.onclick.call(this,event);" onclick="toggleSubordinateFields(this.getElementsByTagName('img')[0],'fes{TITLE|}_help'); return false;" href="#"><img class="inline_image_2 right" alt="{!CONTRACT}: {TITLE*}" title="{!CONTRACT}" src="{$IMG*,contract}" /></a>
			...
			<h2{+START,IF,{$JS_ON}}{+START,IF_NON_PASSED,FORCE_OPEN} onkeypress="return this.onclick.call(this,event);" onclick="toggleSubordinateFields(this.parentNode.getElementsByTagName('img')[0],'fes{TITLE|}_help'); return false;"{+END}{+END}>{TITLE*}</h2>
		{+END}
