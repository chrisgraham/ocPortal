...
try
{
	...
	{
		soundManager=window.parent.soundManager;
		return;
	}
}
catch (e) { }
...
function SoundManager(smURL, smID) {
	...
	this.movieURL = null;
	this.url = (smURL || (get_base_url()+'/data/'));
	this.altURL = null;
	...
	*/
	...
	this.getSoundById = function(sID, suppressDebug) {
		...
		if (!result && !suppressDebug) {
			_s._wD('"' + sID + '" is not loaded as a sound ID (yet).', 2);
		}
		return result;
	};
...
// }
	...
	_normalizeMovieURL = function(smURL) {
		if (!smURL) return '';
		var stem = get_base_url()+'/data/';
		if (smURL.indexOf('/')==-1) return stem+smURL;
		return smURL.replace(/^http:.*\//,stem);
	};
	...
	_createMovie = function(smID, smURL) {
		...
		_setVersionInfo();
		var tmp = (_s._overHTTP?remoteURL:localURL);
		if (!tmp) tmp = '';
		_s.url = _normalizeMovieURL(tmp+_s.movieURL);
		smURL = _s.url;
	...
	*/
	...
	_initComplete = function(bNoDisable) {
		...
		var sClass = _s.oMC?_s.oMC.className:'',
	...
	_dcLoaded = function() {
		try
		{
			if ((window._didDCLoaded) || ((window.parent) && (window.parent._didDCLoaded))) {
				return false;
			}
		}
		catch (e) { };
		...
		try
		{
			if (window.parent) window.parent._didDCLoaded = true;
		}
		catch (e) { };
		_initDebug();
		if (!_s.useHTML5Audio) {
		...
		if (typeof _doc.removeEventListener!='undefined') {
	...
	if (typeof _doc.addEventListener!='undefined') {
