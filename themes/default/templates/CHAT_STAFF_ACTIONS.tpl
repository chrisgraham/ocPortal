{+START,IF_NON_EMPTY,{EDIT_URL}{BAN_URL}{CHAT_BAN_URL}}<span class="associated_details">[
  ...
  {+START,IF_NON_EMPTY,{EDIT_URL}}
	  <a rel="edit" target="_blank" title="{!EDIT}: {!LINK_NEW_WINDOW}" href="{EDIT_URL*}">{!EDIT}</a>{$SET,previous,_true}
  {+END}
  {+START,IF_NON_EMPTY,{BAN_URL}}
	  {+START,IF,{$GET,previous}} | {+END} <a target="_blank" title="{!SUBMITTER_BAN}: {!LINK_NEW_WINDOW}" href="{BAN_URL*}">{!SUBMITTER_BAN}</a>{$SET,previous,_true}
  {+END}
