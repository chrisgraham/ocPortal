	...
	<div class="bottom_under">
		<div class="global_bottom">
			...
			{+START,IF,{$ADDON_INSTALLED,bookmarks}}{+START,IF,{$NOT,{$IS_GUEST}}}
				<span class="global_bottom_button_ref_point" id="bookmarks_menu_rel">&nbsp;</span><a accesskey="n" id="bookmarks_menu_button" onclick="if (typeof window.load_management_menu=='undefined') { require_javascript('javascript_staff'); window.setTimeout(document.getElementById('bookmarks_menu_button').onclick,10); return false; } else return load_management_menu('bookmarks');" href="{$PAGE_LINK*,_SEARCH:bookmarks}&amp;url={$SELF_URL*&}"><img id="bookmarks_menu_img" class="no_theme_img_click" title="{!BOOKMARKS}" alt="{!BOOKMARKS}" src="{$IMG*,bottom/bookmarksmenu}" /></a>&nbsp;
			{+END}{+END}
			...
			{+START,IF,{$HAS_ZONE_ACCESS,adminzone}}
				{+START,IF,{$ADDON_INSTALLED,occle}}{+START,IF,{$HAS_ACTUAL_PAGE_ACCESS,admin_occle}}{+START,IF,{$CONFIG_OPTION,bottom_show_occle_button}}{+START,IF,{$NOT,{$MATCH_KEY_MATCH,adminzone:admin_occle}}}
