...
function realtime_rain_button_load_handler()
{
	...
	var e=document.getElementById('real-time-surround');
	if (e) // Clicked twice - so now we close it
	{
		...
		img.src="{$IMG,bottom/realtime_rain}".replace(/^http:/,window.location.protocol);
		return false;
	}

	img.src="{$IMG,bottom/realtime_rain_off}".replace(/^http:/,window.location.protocol);
	fixImage(img);
	...
	setInnerHTML(x,load_snippet("realtime_rain_load"));
	...
	e.style.top=0;
	e.style.width='100%';
	e.style.height=(getWindowScrollHeight()-40)+'px';
	smoothScroll(0);
...
function start_realtime_rain()
{
	...
	setOpacity(news_ticker,0.5);
	setOpacity(document.getElementById('timer-inner'),0.7);
	news_ticker.style.top='20px';
	news_ticker.style.left=(getWindowWidth()/2-findWidth(news_ticker)/2)+'px';
...
function received_events(ajax_result_frame,ajax_result)
{
	...
	var max_height=findHeight(bubbles.parentNode);
	...
	for (var i=0;i<elements.length;i++)
	{
		if (elements[i].nodeName.toLowerCase()=='div')
		{
			...
			_cloned_message=carefulImportNode(elements[i]);
			...
			setInnerHTML(cloned_message,getInnerHTML(_cloned_message));
			left_pos+=200;
			...
			window.setTimeout(function() {
				...
				cloned_message.style.top=(-(vertical_slot+1)*findHeight(cloned_message))+'px';
				...
				window.setTimeout( function(cloned_message) { return function() {
					if (typeof cloned_message.lines_for!='undefined')
					{
						...
						if ((typeof cloned_message.icon_multiplicity!='undefined') && (!browser_matches('ie')/*Too slow on IE*/))
						{
							...
							var next_icon;
							var icon_spot=document.getElementById('real-time-surround');
							if (findPosY(icon_spot,true)>0) icon_spot=icon_spot.parentNode;
							for (var x=0;x<num;x++)
							{
								window.setTimeout( function(main_icon,next_icon) { return function () {
									...
									setInnerHTML(next_icon,getInnerHTML(main_icon));
									next_icon.style.position='absolute';
									next_icon.style.left=findPosX(main_icon,true)+'px';
									next_icon.style.top=findPosY(main_icon,true)+'px';
									...
									next_icon.animation_timer=window.setInterval(function(next_icon) { return function() {
										...
										setOpacity(next_icon,next_icon.opacity);
...
{
	var pos=window.mouseX-findPosX(document.getElementById('time-line-image'),true);
	...
	if (!prospective)
	{
		...
		bubbles_tidy_up();
		setInnerHTML(document.getElementById('real-time-date'),'{!SET}');
		setInnerHTML(document.getElementById('real-time-time'),'');
		document.getElementById('loading_icon').style.display='block';
	} else
	{
...
function bubbles_tidy_up()
{
	...
	var bubbles=get_elements_by_class_name(document.getElementById('real-time-surround').parentNode,'bubble_wrap');
	...
	setInnerHTML(bubbles_go_here,'');
	window.bubble_groups=[];
	window.total_lines=0;
	var icons=get_elements_by_class_name(document.getElementById('real-time-surround').parentNode,'email-icon');
...
function set_time_line_position(time)
{
	...
	setInnerHTML(realtimedate,date_object.getFullYear()+'/'+(''+date_object.getMonth()).strPad(2,'0',STR_PAD_LEFT)+'/'+(''+date_object.getDate()).strPad(2,'0',STR_PAD_LEFT));
	setInnerHTML(realtimetime,(''+date_object.getHours()).strPad(2,'0',STR_PAD_LEFT)+':'+(''+date_object.getMinutes()).strPad(2,'0',STR_PAD_LEFT)+':'+(''+date_object.getSeconds()).strPad(2,'0',STR_PAD_LEFT));
}
...
function toggle_window_pausing(button)
{
	...
	{
		window.paused=false;
		setOpacity(button,1.0);
	} else
	{
		window.paused=true;
		setOpacity(button,0.7);
	}
}
...
function draw_line(group_id,bubble_id)
{
	...
	} else
	{
		...
		var width=findWidth(ob);
		var height=findHeight(ob);
		var line;
		var x=findPosX(ob,true);
		var y=findPosY(ob,true);
		for (var i=0;i<others.length;i++)
		{
			ob2=document.getElementById(others[i]+'_main');
			if ((ob2) && (ob2.parentNode))
			{
				line=createLine(width/2,height/2,findPosX(ob2,true)+width/2-x,findPosY(ob2,true)+height/2-y,88);
				ob.appendChild(line);
				window.total_lines++;
			}
		}
...
function createLine(x1, y1, x2, y2, margin)
{
	if (x2 < x1)
	{
		var temp = x1;
