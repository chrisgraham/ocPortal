...
<section class="box box___block_main_image_fader_news"><div class="box_inner">
	...
	<div class="image_fader_news_pic">
		<div class="img_thumb_wrap">
			<a id="image_fader_news_url_{$GET,RAND}" href="#"><img id="image_fader_news_{$GET,RAND}" src="{$IMG*,blank}" alt="" /></a>
		</div>
	</div>

	<div class="image_fader_news_html" id="image_fader_news_html_{$GET,RAND}">
...
<script type="text/javascript">// <![CDATA[
	add_event_listener_abstract(window,'load',function () {
		...
		var fp_animation_html=document.getElementById('image_fader_news_html_{$GET,RAND}');
		...
		{$,Pause function}
		window.main_image_fader_news_pause_{$GET,RAND}=function()
		{
			if (window.main_image_fader_news_cycle_timer_{$GET,RAND})
			{
				...
				window.main_image_fader_news_cycle_timer_{$GET,RAND}=null;
				document.getElementById('pause_button_{$GET,RAND}').className='light button_depressed';
			} else
			{
		...
		{$,Cycling function}
		window.main_image_fader_news_cycle_count_{$GET,RAND}=0;
		window.main_image_fader_news_cycle_timer_{$GET,RAND}=null;
		window.main_image_fader_news_cycle_{$GET,RAND}=function(dif)
		{
			{$,Cycle}
			var i=window.main_image_fader_news_cycle_count_{$GET,RAND}+dif;
			if (i<0) i=data.length-1;
			if (i>=data.length) i=0;
			window.main_image_fader_news_cycle_count_{$GET,RAND}=i;
