...
<div class="question_ui_buttons">
	{+START,LOOP,BUTTONS}
		...
		{+START,IF_NON_EMPTY,{$GET,IMG}}
			<a href="#" onclick="window.returnValue='{_loop_var*;}'; window.opener.focus(); if (typeof window.faux_close!='undefined') window.faux_close(); else window.close();"><img class="button_page" src="{$IMG*,page/{$GET,IMG}}" title="" alt="{_loop_var*}" /></a>
		{+END}
