...
function illustrate_frame_load(pf,frame)
{
	{+START,IF,{$CONFIG_OPTION,enable_animations}}
		...
		if (body.length==0)
		{
			set_inner_html(de,'<head>'+head+'<\/head><body aria-busy="true" class="website_body"><div class="spaced"><div class="ajax_loading vertical_alignment"><img id="loading_image" src="'+'{$IMG*;,loading}'.replace(/^http:/,window.location.protocol)+'" alt="{!LOADING;^}" /> <span class="vertical_alignment">{!LOADING;^}<\/span><\/div><\/div><\/body>');
		} else
		{
...
function activate_tooltip(ac,event,tooltip,width,pic,height,bottom,no_delay,lights_off,force_width,win)
{
	...
	{
		if (existing_tooltips[i].id!=ac.tooltip_id) existing_tooltips[i].parentNode.removeChild(existing_tooltips[i]);
	}
...
/* Set up a form to have its CAPTCHA validated upon submission using AJAX */
