...
function illustrate_frame_load(pf,frame)
{
	{+START,IF,{$CONFIG_OPTION,enable_animations}}
		...
		} else
		{
			...
			set_inner_html(body[0],'<div aria-busy="true" class="spaced"><div class="ajax_loading"><img id="loading_image" class="vertical_alignment" src="'+'{$IMG_INLINE*;,loading}'.replace(/^http:/,window.location.protocol)+'" alt="{!LOADING;^}" /> <span class="vertical_alignment">{!LOADING;^}<\/span><\/div><\/div>');
		}
