<div {+START,IF_EMPTY,{CODE}}style="float: none" {+END}class="radio_list_picture{+START,IF_PASSED_AND_TRUE,LINEAR} linear{+END}" onkeypress="if (entered_pressed(event)) return this.onclick.call([event]); else return null;" onclick="this.getElementsByTagName('img')[0].onclick(event);">
	<img class="selectable_theme_image" onkeypress="this.onclick(event);" id="j_{NAME|*}_{CODE|*}_img" onclick="var form=document.getElementById('j_{NAME;|*}_{CODE;|*}').form; var ob=document.getElementById('j_{NAME;|*}_{CODE;|*}_img'); choose_picture('j_{NAME;|*}_{CODE;|*}',ob,'{NAME;*}',event); if (typeof window.main_form_very_simple!='undefined') form.submit(); cancel_bubbling(event);" src="{URL*}" alt="{!SELECT_IMAGE}: {$STRIP_TAGS,{PRETTY*}}" />
