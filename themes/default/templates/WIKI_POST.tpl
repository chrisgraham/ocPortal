...
<div>
	<div class="ocf_topic_post_area ocf_post_main_column wiki_topic_post_area" id="pe_{ID*}">
		{POST}
...
{+START,IF_NON_EMPTY,{BUTTONS}}
	<div>
		<div class="ocf_left_post_buttons post_buttons ocf_post_main_column">
			<div class="buttons_group post_buttons wiki_post_buttons">
				{BUTTONS}

				{+START,IF,{$AND,{$JS_ON},{STAFF_ACCESS}}}
					<div id="cell_mark_{ID*}" class="ocf_off mass_select_marker wiki_mass_select_marker">
						<form class="xhtml_validator_off" title="{!MARKER}: {ID*}" method="post" action="index.php" id="form_mark_{ID*}">
							<div>
								...
								<input onclick="var button=document.getElementById('wiki_merge_button'); button.className=button.className.replace(' button_faded','');" {+START,IF,{$NOT,{$IS_GUEST}}}title="{!MARKER} #{ID*}" {+END}value="1" type="checkbox" id="mark_{ID*}" name="mark_{ID*}" onclick="change_class(this,'cell_mark_{ID*}','ocf_on mass_select_marker','ocf_off mass_select_marker')" />
							</div>
						</form>
					</div>
				{+END}
