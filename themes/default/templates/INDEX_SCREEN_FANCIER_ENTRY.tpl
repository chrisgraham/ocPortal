<div class="box index_screen_fancier_entry"><div class="box_inner">
	{+START,IF_PASSED,IMG}
		{+START,IF_NON_EMPTY,{URL}}<a {+START,IF_PASSED,TARGET}target="{TARGET*}" {+END}href="{URL*}" title="{TITLE*}">{+END}<img class="right float_separation" alt="" src="{IMG*}" />{+START,IF_NON_EMPTY,{URL}}</a>{+END}
	{+END}
