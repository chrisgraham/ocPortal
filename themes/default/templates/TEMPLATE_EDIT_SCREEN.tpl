...
<form title="{!PRIMARY_PAGE_FORM}" method="post" action="{URL*}">
	<div>
		...
		<div class="float_surrounder buttons_group">
			<div class="right">
				<input onclick="disable_button_just_clicked(this); this.form.target='_self'; this.form.action='{URL;*}';" class="buttons__save button_page" type="submit" value="{!SAVE}" />
			</div>
			{+START,IF,{$JS_ON}}
				<div class="right">
					<input onclick="disable_button_just_clicked(this); this.form.target='save_frame'; this.form.action='{URL;*}{$?,{$IN_STR,{URL},?},&amp;,?}save_and_stay=1';" accesskey="U" class="buttons__save_and_stay button_page" type="submit" value="{!SAVE_AND_STAY}" />
				</div>
			{+END}
		</div>
	</div>
