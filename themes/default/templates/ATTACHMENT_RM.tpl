...
<object width="{A_WIDTH*}" height="{A_HEIGHT*}" type="audio/x-pn-realaudio" classid="clsid:CFCDAA03-8BE4-11cf-B84B-0020AFBBCCFA">
	...
	<!--[if !IE]> -->
		<object width="{A_WIDTH*}" height="{A_HEIGHT*}" data="{SCRIPT*}?id={ID*}{+START,IF_PASSED,SUP_PARAMS}{SUP_PARAMS*}{+END}{$KEEP*,0,1}&amp;for_session={$SESSION_HASHED*}" type="audio/x-pn-realaudio">
			<param name="src" value="{SCRIPT*}?id={ID*}{+START,IF_PASSED,SUP_PARAMS}{SUP_PARAMS*}{+END}{$KEEP*,0,1}&amp;for_session={$SESSION_HASHED*}" />
			<param name="quality" value="high" />
			<param name="autostart" value="false" />
			<param name="controls" value="ControlPanel" />
