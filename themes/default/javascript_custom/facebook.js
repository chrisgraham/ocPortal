...
function facebook_init(app_id,channel_url,just_logged_out,serverside_fbuid,home_page_url,logout_page_url)
{
	window.fbAsyncInit=function() {
		...
		if (serverside_fbuid===null) // If not already in an ocPortal Facebook login session we may need to listen for explicit new logins
		{
			FB.Event.subscribe('auth.login',function(response) { // New login status arrived - so an ocPortal Facebook login session should be established, or ignore as we are calling a logout within this request (above)
				...
				{
					// ... and therefore refresh to let ocPortal server-side re-sync, as this was a new login initiated just now on the client side
