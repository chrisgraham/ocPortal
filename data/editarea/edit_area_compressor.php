...
if (class_exists('Compressor')) // 'if' is for the code quality checker
...
class Compressor{
	...
	function check_cache()
	{
		...
		if (file_exists($this->cache_file))
		{
			...
			$cache_date=0;				
			if (($dir = opendir($this->path))!==false)
			{
			...
			if(filemtime($this->cache_file) >= $cache_date)
			{
				...
				{
					header("Last-modified: ".strval($last_modified));
					header("Cache-Control: Public"); // Tells HTTP 1.1 clients to cache
					header("Pragma:"); // Tells HTTP 1.0 clients to cache
					header('Content-Length: '.strval(filesize($this->cache_file)));
		...
		if (($dir = @opendir($reg_path)) !== false)
		{
			while (($file = readdir($dir)) !== false)
			{
				if( $file !== "." && $file !== ".." && ( $pos = strpos( $file, '.js' ) ) !== false )
				{
					...
					{
						$a_display_name[] = "'". substr( $file, 0, $pos ) ."':'". htmlentities( $match[3], ENT_QUOTES ) ."'";
					}
				}
			}
			closedir($dir);
	...
	function send_datas()
	{
		if($this->param['debug'])
		{
			...
			{
				$ratio = round(100.0 - floatval(strlen($this->datas)) / floatval($this->file_loaded_size) * 100.0);
			}
...
//		die;
	...
	function replace_scripts($var, $param1, $param2)
	{
		if ($var=='script_list')
		{
			$this->script_list=stripslashes($param2);
		} else
		{
