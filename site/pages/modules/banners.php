...
class Module_banners
{
	...
	function choose_banner()
	{
		...
		$rows=$GLOBALS['SITE_DB']->query_select('banners',array('*'),$only_owned,'ORDER BY '.$current_ordering,$max,$start);
		foreach ($rows as $row)
		{
		...
		$table=results_table(do_lang('BANNERS'),get_param_integer('start',0),'start',get_param_integer('max',300),'max',$max_rows,$header_row,$fields,$sortables,$sortable,$sort_order);
