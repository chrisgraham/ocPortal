...
class Module_lostpassword
{
	...
	function step2()
	{
		...
		$message=do_lang($temporary_passwords?'RESET_PASSWORD_TEXT_TEMPORARY':'RESET_PASSWORD_TEXT',comcode_escape(get_site_name()),comcode_escape($username),array(comcode_escape($url),$url_simple,strval($member),strval($code)),get_lang($member));
		require_code('mail');
