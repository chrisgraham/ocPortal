		...
		{+START,IF_NON_EMPTY,{TITLE}}
			<h3 class="standardbox_title_classic" onkeypress="this.onclick(event);" onclick="this.getElementsByTagName('a')[0].onclick(event);">
				{+START,IF_NOT_IN_ARRAY,tray_open,OPTIONS}
					{+START,IF,{$JS_ON}}<a class="standardbox_tray hide_button" href="#" onclick="accordion(this.parentNode.parentNode); cancel_bubbling(event,this); return false;"><img alt="{!EXPAND}" title="{!EXPAND}" src="{$IMG*,expand}" /></a> {+END}
				{+END}
				{+START,IF_IN_ARRAY,tray_open,OPTIONS}
					{+START,IF,{$JS_ON}}<a class="standardbox_tray hide_button" href="#" onclick="accordion(this.parentNode.parentNode); cancel_bubbling(event,this); return false;"><img alt="{!CONTRACT}" title="{!CONTRACT}" src="{$IMG*,contract}" /></a> {+END}
				{+END}
				{TITLE}
			</h3>
		{+END}
