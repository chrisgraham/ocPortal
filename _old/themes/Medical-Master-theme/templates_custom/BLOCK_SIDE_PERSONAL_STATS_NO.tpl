{+START,IF,{$NOR,{$GET,login_screen},{$MATCH_KEY_MATCH,_WILD:login}}}
	{+START,BOX,{TITLE},,{$?,{$GET,in_panel},panel,classic}}
		<form title="{!_LOGIN}" onsubmit="if (check_fieldForBlankness(this.elements['login_username'],event)) { disable_button_just_clicked(this); return true; } return false;" action="{LOGIN_URL*}" method="post" class="autocomplete side_block_form">
			<div>
				...
				<div class="constrain_field"><input class="more" type="submit" value="{!_LOGIN}" /></div>
			</div>
		</form>
