			...
			<h2 class="standardbox_title_panel">
				{+START,IF,{$CONFIG_OPTION,tray_support}}{+START,IF,{$JS_ON}}
					{+START,IF_IN_ARRAY,tray_open,OPTIONS}
						<a class="standardbox_tray hide_button" href="#" onclick="SetCookie('tray_{TITLE|}',(get_elements_by_class_name(this.parentNode.parentNode,'hide_tag')[0].style.display=='none')?'open':'closed'); hide_tag(this.parentNode.parentNode); /*resize_frame('the_shoutbox');*/ return false;"><img alt="{!CONTRACT}: {$STRIP_TAGS,{TITLE}}" title="{!CONTRACT}" src="{$IMG*,contract}" /></a>
					{+END}
					{+START,IF_IN_ARRAY,tray_closed,OPTIONS}
						<a class="standardbox_tray hide_button" href="#" onclick="SetCookie('tray_{TITLE|}',(get_elements_by_class_name(this.parentNode.parentNode,'hide_tag')[0].style.display=='none')?'open':'closed'); hide_tag(this.parentNode.parentNode); /*resize_frame('the_shoutbox');*/ return false;"><img alt="{!EXPAND}: {$STRIP_TAGS,{TITLE}}" title="{!EXPAND}" src="{$IMG*,expand}" /></a>
					{+END}
				{+END}{+END}
				{TITLE}
			</h2>
