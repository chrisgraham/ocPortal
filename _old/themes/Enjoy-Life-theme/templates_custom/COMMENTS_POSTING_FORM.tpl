...
<form{$?,{$VALUE_OPTION,html5}, role="form"} title="{TITLE*}" id="comments_form" onsubmit="return ({+START,IF_PASSED,MORE_URL}(this.getAttribute('action')=='{MORE_URL*}') || {+END}(check_fieldForBlankness(this.elements['post'],event)){+START,IF,{$AND,{GET_EMAIL},{$NOT,{EMAIL_OPTIONAL}}}} &amp;&amp; (check_fieldForBlankness(this.elements['email'],event)){+END});" action="{COMMENT_URL*}{+START,IF_PASSED,COMMENTS}#last_comment{+END}" method="post" enctype="multipart/form-data">
			...
			<div class="standardbox_classic">
				<h3 class="standardbox_title_classic toggle_div_title">
					...
					{+START,IF_PASSED,EXPAND_TYPE}
						&nbsp; <a class="hide_button hide_button_spacing" href="#" onclick="event.returnValue=false; toggle_section_inline('comments_posting_form_outer','block'); return false;"><img id="e_comments_posting_form_outer" alt="{$?,{$EQ,{EXPAND_TYPE},contract},{!CONTRACT},{!EXPAND}}" title="{$?,{$EQ,{EXPAND_TYPE},contract},{!CONTRACT},{!EXPAND}}" src="{$IMG*,{EXPAND_TYPE*}}" /></a>
					{+END}
				</h3>
			</div>
		{+END}
					...
					<div class="contact-form">
						<ul>
							...
							<li>
								...
								<textarea accesskey="x" class="wide_field" onfocus="if ((this.value.replace(/\s/g,'')=='{POST_WARNING^*;}'.replace(/\s/g,'') &amp;&amp; '{POST_WARNING^*;}'!='') || (typeof this.strip_on_focus!='undefined' &amp;&amp; this.value==this.strip_on_focus)) this.value=''; this.style.color='black';" cols="35" rows="11" name="post" id="post">{POST_WARNING*}</textarea>
							</li>
						</ul>
					</div>
				{+END}
						...
						<tbody>
												...
												<script type="text/javascript">// <![CDATA[
													function new_review_highlight__{TYPE%}__{$FIX_ID,{REVIEW_TITLE}}__{ID%}(review,first_time)
													{
														...
														for (i=1;i<=5;i++)
														{
															...
															set_opacity(bit,((review!=0) && (review/2>=i))?1.0:0.2);
							...
							<tr>
									...
									{+START,IF_NON_EMPTY,{FIRST_POST}{COMMENT_TEXT}}
										<div class="comments_posting_form_links">
											...
											{+START,IF_NON_EMPTY,{FIRST_POST}}
												<a class="non_link" title="{!ocf:FIRST_POST}: {!LINK_NEW_WINDOW}" target="_blank" href="{FIRST_POST_URL*}" onmousemove="if (typeof window.activate_tooltip!='undefined') reposition_tooltip(this,event);" onmouseout="if (typeof window.deactivate_tooltip!='undefined') deactivate_tooltip(this,event);" onblur="this.onmouseout(event);" onfocus="this.onmouseover(event);" onmouseover="if (typeof window.activate_tooltip!='undefined') activate_tooltip(this,event,'{FIRST_POST^;~*}','30%',null,null,false,true);">{!ocf:FIRST_POST}</a>
											...
											{+START,IF_NON_EMPTY,{COMMENT_TEXT}}
												<a class="non_link" href="{$PAGE_LINK*,:rules}" onmousemove="if (typeof window.activate_tooltip!='undefined') reposition_tooltip(this,event);" onmouseout="if (typeof window.deactivate_tooltip!='undefined') deactivate_tooltip(this,event);" onblur="this.onmouseout(event);" onfocus="this.onmouseover(event);" onmouseover="if (typeof window.activate_tooltip!='undefined') activate_tooltip(this,event,'{$TRUNCATE_LEFT,{COMMENT_TEXT^;~*},1000,0,1}','30%',null,null,false,true);">{!HOVER_MOUSE_IMPORTANT}</a>
											{+END}
											)
										</div>
									{+END}
								...
								<td>
									...
									<div class="constrain_field">
										<textarea{+START,IF,{$NOT,{$MOBILE}}} onkeyup="manage_scroll_height(this);"{+END} accesskey="x" class="wide_field" onfocus="if ((this.value=='{POST_WARNING^*;}' &amp;&amp; '{POST_WARNING^*;}'!='') || (typeof this.strip_on_focus!='undefined' &amp;&amp; this.value==this.strip_on_focus)) this.value=''; this.style.color='black';" tabindex="3" cols="35" rows="11" name="post" id="post">{POST_WARNING*}</textarea>
									</div>
