...
$s_credit_value = is_null($result = get_option('support_credit_value',true))? 5.5: integer_format($result);
$budget_minutes = is_null($result = get_option('support_budget_priority',true))? 10: integer_format($result);
$multi_rate = round(intval(60/$budget_minutes)) < 1? 1: round(intval(60/$budget_minutes));
...
if (isset($map['sort']))
{
	...
	switch ($sort)
	{
		...
		case 'sponsorship_progress':
			...
			$order='(SELECT SUM(amount) FROM mantis_sponsorship_table z WHERE z.bug_id=a.id)/CAST(c.value AS DECIMAL)*'.strval($multi_rate).'*'.strval($s_credit_value).' '.$direction;
			break;
	}
}
...
$select.=',CAST(c.value AS DECIMAL)*'.strval($multi_rate).'*'.strval($s_credit_value).' AS currency_needed';
$table='mantis_bug_table a JOIN mantis_bug_text_table b ON b.id=a.bug_text_id JOIN mantis_custom_field_string_table c ON c.bug_id=a.id AND field_id='.$ocp_hours_field.' JOIN mantis_category_table d ON d.id=a.category_id';
...
} else
{
	...
	foreach ($issues as $issue)
	{
		...
		$cost=($issue['hours']==0 || is_null($issue['hours']))?NULL:($issue['hours']*$s_credit_value*$multi_rate);
		...
		echo static_evaluate_tempcode(put_in_standard_box($out,$title,NULL,'curved'));
	}
...
$results_browser=pagination('Issues',NULL,$start,'start',$max,'max',$max_rows,NULL,NULL,true);
