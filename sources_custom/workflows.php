...
function can_choose_workflow($user=NULL)
{
	...
	// criterion
	return has_actual_page_access($user,'admin_workflow',get_module_zone('admin_workflow'));
}
...
function workflow_update_handler()
{
	...
	require_code('hooks/systems/content_meta_aware/'.filter_naughty_harsh($content_details[0]['content_type']));
	$ob=object_factory('Hook_content_meta_aware_'.filter_naughty_harsh($content_details[0]['content_type']),true);
	...
	if (($content_is_validated[0][$content_validated_field]==1)!=$all_points_approved)
	{
		...
		$GLOBALS['SITE_DB']->query_update($content_table,array($content_validated_field=>$all_points_approved?1:0),array($content_field=>$content_field=>$info['id_field_numeric']?intval($content_details[0]['content_id']):$content_details[0]['content_id']),'',1);
	}
