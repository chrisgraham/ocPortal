...
function server__create_forum_topic($forum_id,$topic_title,$post)
{
	...
	require_code('ocf_posts_action');
	require_code('mantis'); // Defines LEAD_DEVELOPER_MEMBER_ID
	$topic_id=ocf_make_topic($forum_id);
	$post_id=ocf_make_post($topic_id,$topic_title,$post,0,true,1,0,NULL,NULL,NULL,LEAD_DEVELOPER_MEMBER_ID);
...
function do_backup_script()
{
	...
	$tmp_path=tempnam((((ini_get('safe_mode')=='1') || ((ini_get('open_basedir')!='') && (strpos(ini_get('open_basedir'),'/tmp')===false)))?get_custom_file_base().'/uploads/attachments/':'/tmp/'),'');
