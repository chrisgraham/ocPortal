...
function find_activities($viewer_member,$mode,$member_ids)
{
	...
	if (addon_installed('chat'))
	{
		if (!$is_guest) // If not a guest, get all blocks
		{
			...
			$blocked_by=$GLOBALS['SITE_DB']->query_select('chat_blocking',array('member_blocker'),array('member_blocked'=>$viewer_member));
			...
			$blocking=$GLOBALS['SITE_DB']->query_select('chat_blocking',array('member_blocked'),array('member_blocker'=>$viewer_member));
				...
				if (($member_id!=$viewer_member) && addon_installed('chat'))
				{
					...
					if ((!$is_guest))
					{
						if (strlen($blocked_by)>0) // On the basis that you've sought this view out, your blocking them doesn't hide their messages.
							$friends_check_where='(member_likes='.strval($member_id).' AND member_liked='.strval($viewer_member).' AND member_likes NOT IN('.$blocked_by.'))';
						else
		...
		case 'friends':
			...
			if (addon_installed('chat') && !$is_guest) // If not a guest, get all reciprocal friendships.
			{
				...
				if (count($like_mutual)>1) // More than one mutual friend
				{
					...
					$like_outgoing=$GLOBALS['SITE_DB']->query_select('chat_friends',array('member_liked'),NULL,' WHERE (member_likes='.strval($viewer_member).' AND member_liked NOT IN('.$lm_ids.')'.$extra_not);
				...
				{
					$like_outgoing=$GLOBALS['SITE_DB']->query_select('chat_friends',array('member_liked'),NULL,' WHERE (member_likes='.strval($viewer_member).' AND member_liked!='.strval($like_mutual[0]['liked']).$extra_not);
				...
				{
					if (!$is_guest)
						$like_outgoing=$GLOBALS['SITE_DB']->query_select('chat_friends',array('member_liked'),NULL,' WHERE (member_likes='.strval($viewer_member).$extra_not);
