...
/**
 * Give the specified workflow a new requirement.
...
function add_requirement_to_workflow($bits,$workflow_id,$position=NULL)
{
	...
	if (is_null($position))
	{
		...
		$current_position=$GLOBALS['SITE_DB']->query_value('workflow_requirements','MAX(the_position)',array('workflow_id'=>$workflow_id));
	...
	return $GLOBALS['SITE_DB']->query_insert('workflow_requirements',$map,true);
}
...
function delete_workflow($id)
{
	...
	$points=$GLOBALS['SITE_DB']->query_select('workflow_requirements',array('id','workflow_approval_name'),array('workflow_id'=>$id));
	foreach ($points as $p)
	{
		delete_lang($p['workflow_approval_name']);
		$GLOBALS['SITE_DB']->query_delete('workflow_permissions',array('workflow_requirement_id'=>$p['id']));
	}

	// Now remove the workflow from the database and remove its association with
	// approval points
	delete_lang($GLOBALS['SITE_DB']->query_value('workflows','workflow_name',array('id'=>$id)));
	$GLOBALS['SITE_DB']->query_delete('workflows',array('id'=>$id),'',1);
	$GLOBALS['SITE_DB']->query_delete('workflow_requirements',array('workflow_id'=>$id));
}
 ...
 *
 * @param  AUTO_LINK		The workflow approval point ID
 */
function delete_approval_point($name)
{
	// Grab all content awaiting this approval
	$content=$GLOBALS['SITE_DB']->query_select('workflow_content_status',array('status_code','workflow_content_id'),array('workflow_requirement_id'=>$name));
	...
	$affected_workflows=$GLOBALS['SITE_DB']->query_select('workflow_requirements',array('workflow_id'),array('workflow_approval_name'=>$name));
	foreach ($affected_workflows as $workflow)
	{
		...
		if (count(get_requirements_for_workflow($workflow['workflow_id']))==1)
		{
			delete_workflow($workflow['workflow_id']);
		}
		// Otherwise we can just remove this one point
		else
		{
			$GLOBALS['SITE_DB']->query_delete('workflow_requirements',array('workflow_approval_name'=>$name,'workflow_id'=>$workflow['workflow_id']),'',1);
		}
	}

	// Now we remove the permissions associated with this approval point
	$GLOBALS['SITE_DB']->query_delete('workflow_permissions',array('workflow_requirement_id'=>$name));
}
