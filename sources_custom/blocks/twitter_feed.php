	...
	function run($map)
	{
		...
		foreach ($twitter_statuses as $status)
		{
			...
			if (is_numeric($time_ago_timestamp) && $time_ago_timestamp>0)
			{
				...
				//if less then minute
				elseif ($time_ago_timestamp<$minute) $time_ago=strval($time_ago_timestamp).' seconds ago';

				//if less then 2 minutes
				elseif ($time_ago_timestamp<$minute*2) $time_ago='about 1 minute ago';

				//if less then hour
				elseif ($time_ago_timestamp<$hour) $time_ago=strval(intval(floor($time_ago_timestamp/$minute))).' minutes ago';

				//if less then 2 hours
				elseif ($time_ago_timestamp<$hour*2) $time_ago='about 1 hour ago';

				//if less then day
				elseif ($time_ago_timestamp<$day) $time_ago=strval(intval(floor($time_ago_timestamp/$hour))).' hours ago';

				//if more then day, but less then 2 days
				elseif ($time_ago_timestamp>$day && $time_ago_timestamp<$day*2) $time_ago='yesterday';

				//if less then year
				elseif ($time_ago_timestamp<$day*365) $time_ago=strval(intval(floor($time_ago_timestamp/$day))).' days ago';
