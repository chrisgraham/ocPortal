	...
	function run($map)
	{
		...
		if (isset($result->feed->entry))
		{
			...
			{ 
				...
				if ($ratingstotal>0) $likespercent=intval(round(($likes/$ratingstotal)*100));			//generate percentage of likes
				...
				if ($rating>0) $ratingstars=intval(round($rating));				//generate full star rating of 1 to 5 stars (no half stars)
