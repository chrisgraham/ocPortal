...
class Block_youtube_channel
{
    ...
    function uninstall()
    {
        ...
        delete_config_option('youtube_channel_block_api_key');
    }
    ...
    function run($map)
    {
        ...
        if ($channel_name == '') $playlist_id=array_key_exists('playlist_id',$map)?trim($map['playlist_id']):'';
        else $playlist_id = '';
        ...
        if (!$youtube_api_key) $channel_error .= 'Error: No YouTube API Key has been specified.<br>';

        // Sanity checks on some critical input - if out of range or unknown values are detected, set them to a default value
        if ($channel_startvideo > 1) $channel_maxvideos = $channel_maxvideos + ($channel_startvideo-1);
        if ($channel_maxvideos < 1 || $channel_maxvideos > 50) $channel_maxvideos = 50;
        if ($channel_startvideo < 1 || $channel_startvideo > 50) $channel_startvideo = 1;
        if ($channel_orderby < 1 || $channel_orderby > 3) $channel_orderby = 1; //left in for backward compatibility, but this is not available for the playlistItems.list API call in the new YouTube API v3
        if ($channel_showplayer < 0 || $channel_showplayer + $channel_maxvideos > 50) $channel_showplayer = 50;
        if (!in_array($channel_playeralign, array('center', 'left', 'right'))) $channel_playeralign = 'center';
        if ($channel_nothumbplayer < 0 || $channel_nothumbplayer > 1) $channel_nothumbplayer = 0;
        if ($channel_thumbnail < 0 || $channel_thumbnail > 7) $channel_thumbnail = 0;
        if (!in_array($channel_descriptiontype, array('long', 'short'))) $channel_descriptiontype = 'long';

        // Create some working variables for working in foreach loop
        if (($channel_showplayer + $channel_startvideo - 1) >= $channel_maxvideos) $temp_showplayer = $channel_maxvideos;
        else $temp_showplayer = $channel_showplayer + $channel_startvideo - 1;
        ...
        if ($channel_name) {
            ...
            if (isset($channel->items[0]->contentDetails->relatedPlaylists->uploads)) $playlist_id = $channel->items[0]->contentDetails->relatedPlaylists->uploads;
            else $channel_error .= 'Error: Unable to get uploads playlist for <b>'.$channel_name.'</b>. Verify you have the correct user name and API key.<br>';

            // Set channel url using user name
            $channel_url='http://www.youtube.com/user/'.$channel_name;
        }
        if (!$playlist_id) $channel_error .= 'Error: No channel user name or playlist ID specified.<br>';
        ...
        if (isset($playlist_items->pageInfo->totalResults)) $total_playlist_items = $playlist_items->pageInfo->totalResults;
        else $total_playlist_items = 0;
        if ($channel_startvideo > $total_playlist_items) $channel_error .= 'Error: start_video parameter is beyond the end of the playlist.<br>';
        ...
        if (isset($playlist_items->items) && $channel_startvideo <= $total_playlist_items) {
            foreach ($playlist_items->items as $entry) {
                ...
                $is_public = $entry->status->privacyStatus;                                                            //get private/public status of video
                ...
                if (($is_public == 'public') && ($i >= $channel_startvideo) && ($i <= $channel_maxvideos)) {
                    $title = (isset($entry->snippet->title)?$entry->snippet->title:'');                                //get video title
                    $description_long = (isset($entry->snippet->description)?$entry->snippet->description:'');         //get video description
                    $uploaded = $entry->snippet->publishedAt;                                                          //get video upload date/time
                    $video_id = $entry->snippet->resourceId->videoId;                                                  //get video id
                    $thumbnails = $entry->snippet->thumbnails;                                                         //get video thumbnails
                    $video_url = 'https://www.youtube.com/watch?v='.$video_id;                                         //generate video url
                    
                    // Generate shortened description
                    $description_shortened = 0;
                    if (strlen($description_long) <= 250) $description_short = $description_long;
                    else {
                    ...
                    if ($ratingstotal > 0) $rating = round($likespercent/20,2);                 //if video has likes or dislikes, get the average rating (can be used for half star ratings of 1 to 5 stars)
                    else $rating = 0;                                                         //if video has no likes or dislikes, node will not be available - manually set rating is set to 0

                    if ($ratingstotal > 0) $ratingstars = round($rating);                     //generate full star rating of 1 to 5 stars (no half stars)
                    else $ratingstars = 0;                                                    //if no likes or dislikes, set full star rating to 0

                    $hours = 0; $minutes = 0; $seconds = 0;                                   //initialise hours, minutes, and seconds to 0
                    $duration_text = '';                                                      //initialise duration_text (blanks the variable each run through the loop)
                    $video_duration = $videoMetadata->items[0]->contentDetails->duration;     //get video duration from YouTube API
                    ...
                    if ($seconds == 0 || $seconds > 1) $seconds_text = "$seconds seconds";    //if more than one second or 0, use plural - seconds
                    else $seconds_text = "$seconds second";                                   //if only one second, use singular - second
                    if ($minutes > 1) $minutes_text = "$minutes minutes";                     //if more than one minute, use plural - minutes
                    elseif ($minutes == 1) $minutes_text = "$minutes minute";                 //if only one minute, use singular - minute
                    else $minutes_text = NULL;                                                //if 0 minutes, we will omit text
                    if ($hours > 1) $hours_text = "$hours hours";                             //if more than one hour, use plural - hours
                    elseif ($hours == 1) $hours_text = "$hours hour";                         //if only one hour, use singular - hour
                    else $hours_text = NULL;                                                  //if 0 hours, we will omit text

                    if ($hours_text) $duration_text = $hours_text;
                    if ($duration_text && $minutes_text) $duration_text = $duration_text.", ".$minutes_text;
                    else $duration_text = $minutes_text;
                    if ($duration_text && $seconds_text) $duration_text = $duration_text.", ".$seconds_text;
                    else $duration_text = $seconds_text;
                    ...
                    if ($hours < 24) {
                        ...
                        $duration_numeric = $numeric->format('H:i:s');                           //generate numeric time format hh:mm:ss
                    }
                    else {
                        $numeric = new DateTime('@0');
                        $numeric->add(new DateInterval('PT'.$minutes.'M'.$seconds.'S'));
                        $duration_numeric = sprintf('%02d',$hours).':'.$numeric->format('i:s:'); //generate numeric time format mm:ss
                    }

                    $embedvideo = 'https://www.youtube.com/embed/' . $video_id;                  //generate embedded video player url
                    ...
                    $base_thumb_url = dirname($thumbnails->default->url).'/';                    //set base url for thumbnails to use for thumbnails that are no longer returned by API call
        ...
        else $channel_error .= 'Error: Invalid playlist ID, no data returned, or possibly the YouTube API request limit is exceded.<br>';  //set error if channel request doesn't return a channel result
