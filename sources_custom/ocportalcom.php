...
function myocp_add_site_raw($server, $codename, $email_address, $password)
{
    ...
    $cmd = 'mysql -h' . /*$server*/
        'localhost' . ' -Dmyocp_site_' . $codename . ' -u' . $user . ' -p' . $GLOBALS['SITE_INFO']['mysql_myocp_password'] . ' < ' . special_myocp_dir() . '/template.sql';
    if (get_member() == 6) {
...
{
    return 'uploads/website_specific/ocportal.com/myocp';
}
...
{
    if (!file_exists(special_myocp_dir() . '/servers')) {
        return array('');
    }
...
\$SITE_INFO['custom_file_base_stub']='/home/ocp/public_html/servers/myocp.com/sites';
\$SITE_INFO['custom_share_domain']=(strpos(\$_SERVER['HTTP_HOST'],'.3c.ms')!==false)?'3c.ms':'myocp.com';
...
function reset_aliases()
{
    ...
    foreach ($sites as $site) {
        ...
        if ($site['s_domain_name'] != '') {
            $text .= $site['s_domain_name'] . ':' . 'alias-myocp_' . $site['s_codename'] . "\n";
        }
    }
    $myfile = fopen(special_myocp_dir() . '/virtualdomains', GOOGLE_APPENGINE ? 'wb' : 'at');
    @flock($myfile, LOCK_EX);
    if (!GOOGLE_APPENGINE) {
        ftruncate($myfile, 0);
    }
    ...
    $myfile = fopen(special_myocp_dir() . '/rcpthosts', GOOGLE_APPENGINE ? 'wb' : 'at');
    @flock($myfile, LOCK_EX);
    if (!GOOGLE_APPENGINE) {
        ftruncate($myfile, 0);
    }
    ...
    foreach ($emails as $email) {
        $myfile = fopen($a_path . '/.qmail-myocp_' . filter_naughty($email['s_codename']) . '_' . filter_naughty(str_replace('.', ':', $email['s_email_from'])), GOOGLE_APPENGINE ? 'wb' : 'at');
        @flock($myfile, LOCK_EX);
        if (!GOOGLE_APPENGINE) {
            ftruncate($myfile, 0);
        }
...
function do_backup_script()
{
    ...
    shell_exec('mysqldump -h' . /*$server*/
        'localhost' . ' -u' . $user . ' -p' . $GLOBALS['SITE_INFO']['mysql_myocp_password'] . ' myocp_site_' . $id . ' --skip-opt > ' . $tmp_path);
