...
class upon_query_tracking_galleries
{
	/**
	 * Standard query hook
	 *
	...
	*/
	function run($ob,$query,$max,$start,$fail_ok,$id,$ret)
	{
		...
		$table='(images|videos)';
		$type='galleries';
		$view_page_name='galleries';
		$matches=array();
		if (preg_match('#^(UPDATE|INSERT INTO) '.$ob->get_table_prefix().$table.'#',$query,$matches)!=0)
		{
			$cat=post_param('cat','');
			if ($cat=='') return;

			require_code('tracking');
			$view_type=($matches[2]=='images')?'image':'video';
