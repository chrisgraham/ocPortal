...
function xml_dump_script()
{
	...
	if (get_db_type()!='xml') warn_exit('It makes no sense to run this script if you are not running the XML database driver.');
	global $SITE_INFO;
	...
	if (strtolower(ocp_srv('REQUEST_METHOD'))=='get') // Interface
	{
		...
		echo '<p><input class="menu___generic_admin__sync button_page" type="submit" value="Sync" /> &nbsp;&nbsp;&nbsp;&nbsp; [<a href="#" onclick="var form=document.getElementsByTagName(\'form\')[0]; for (var i=0;i&lt;form.elements.length;i++) if (form.elements[i].checked) form.elements[i].checked=false; return false;">un-tick all</a>]</p>';
		echo '</form>';
...
function find_all_tables($db)
{
	...
	{
		if (!isset($tables[$field['m_table']])) $tables[$field['m_table']]=array();
