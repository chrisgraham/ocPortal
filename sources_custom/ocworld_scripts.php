...
function ocworld_messages_script()
{
	...
	foreach ($rows as $myrow)
	{
		...
		if (is_null($message_sender)) $message_sender=do_lang('UNKNOWN');
...
function download_map($realm,$sx,$sy)
{
	...
	for ($x=$x_min;$x<=$x_max;$x++)
	{
		for ($y=$y_min;$y<=$y_max;$y++)
		{
			...
			if (!array_key_exists(0,$rooms)) continue;
			$room=$rooms[0];
			$name=$room['name'];
			...
			if (is_null($owner)) $owner=do_lang('UNKNOWN');

			// Draw room
			if (($x==$sx) && ($y==$sy)) imagerectangle($my_img,$ax+3,$ay+3,$bx-3,$by-3,$cucolor);
