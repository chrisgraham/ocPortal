...
function get_innodb_table_sql($tables,$all_tables)
{
	...
	for ($loop_it=0;$loop_it<count($tables);$loop_it++)
	{
		...
		foreach ($fields as $field=>$type)
		{
			...
			if (strpos($type,'*')!==false) $keys[]=$field;
			...
			if (isset($relations[$table.'.'.$field]))
			{
				...
				if (!isset($tables[$mapped_table])) $tables[$mapped_table]=$all_tables[$mapped_table];
			}
		}
		$out.="			PRIMARY KEY (";
		...
		foreach ($files as $file)
		{
			if ((strpos($file,'blocks/')!==false) || (strpos($file,'pages/modules')!==false))
			{
				...
				for ($i=0;$i<$num_matches;$i++)
				{
					...
					{
						if ($table_name=='group_page_access') $addon_name='core';
						if ($table_name=='group_zone_access') $addon_name='core';

						if (!isset($tables_by[$addon_name])) $tables_by[$addon_name]=array();
...
function get_table_descriptions()
{
	$table_descriptions=array(
		...
		'long_values'=>'arbitrary store of long data values (mapping of keys to values)',
...
function get_relation_map()
{
	$relation_map=array(
		...
		'seedy_changes.the_page'=>'seedy_pages.id',
		'seedy_children.parent_id'=>'seedy_pages.id',
		'seedy_children.child_id'=>'seedy_pages.id',
		'seedy_posts.page_id'=>'seedy_pages.id',
