...
function install_location_data()
{
	...
	while (($line=fgetcsv($myfile,4096))!==false)
	{
		...
		if ($newline['Country']=='UK') $newline['Country']='United Kingdom';
		if ($newline['Country']=='Reunion') $newline['Country']='France';
	...
	while (($line=fgetcsv($myfile,4096))!==false)
	{
		...
		// Fix inconsistencies
		if ($newline['Country']=='United States') $newline['Country']='United States of America';
...
function transcode_remaining_locations()
{
	...
	$type='yahoo'; // Either google or yahoo or bing or mapquest
		...
		} else
		{
			foreach ($unknown as $location)
			{
				...
				if ((($type=='bing') && (preg_match('#<Latitude>([\-\d\.]+)</Latitude>\s*<Longitude>([\-\d\.]+)</Longitude>#',$result,$matches)!=0)) || (($type=='google') && (preg_match('#<lat>([\-\d\.]+)</lat>\s*<lng>([\-\d\.]+)</lng>#',$result,$matches)!=0)) || (($type=='yahoo') && (preg_match('#<latitude>([\-\d\.]+)</latitude>\s*<longitude>([\-\d\.]+)</longitude>#',$result,$matches)!=0)))
				{
					...
					$errored=0;
				} elseif (preg_match('#(ZERO_RESULTS|<StatusCode>200</StatusCode>)#',$result)==0) /*probably hit an API limit, or connection problem*/$errored++;
...
function create_catalogue_category_tree($catalogue_name='places',$country=NULL,$fixup=false/*used to fix old bug where countries without regions were not imported*/)
{
	...
	foreach ($locations as $location)
	{
		...
		if ($location['l_parent_1']!='') $tree_pos[]=$location['l_parent_1'];
		if ($location['l_parent_2']!='') $tree_pos[]=$location['l_parent_2'];
		if ($location['l_parent_3']!='') $tree_pos[]=$location['l_parent_3'];
...
function _create_catalogue_subtree($fields,$first_cat,$catalogue_name,$root_cat,$full_tree,$node,$tree_pos)
{
	...
	if (is_null($node['details']['l_latitude'])) $node['details']['l_latitude']=($node['details']['l_min_latitude']+$node['details']['l_max_latitude'])/2;
	if (is_null($node['details']['l_longitude'])) $node['details']['l_longitude']=($node['details']['l_min_longitude']+$node['details']['l_max_longitude'])/2;
