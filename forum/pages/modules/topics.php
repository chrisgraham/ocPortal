...
class Module_topics
{
	...
	{
		return is_guest()?array():array('new_pt'=>'ADD_PRIVATE_TOPIC');
	}
...
//		if (is_null($from)) warn_exit(do_lang_tempcode('MISSING_RESOURCE'));
	...
	function attach_quotes($quotes)
	{
		...
		foreach ($quotes as $quote)
		{
			...
			$post->attach(do_template('OCF_QUOTE_FCOMCODE',array('_GUID'=>'5542508cad43a0cd5798afbb06f9e616','ID'=>strval($quote),'TITLE'=>$_topic[0]['t_cache_first_title'],'POST'=>preg_replace('#\[staff_note\].*\[/staff_note\]#Us','',get_translated_text($_postdetails[0]['p_post'],$GLOBALS['FORUM_DB'])),'BY'=>$_postdetails[0]['p_poster_name_if_guest'])));
		}
...
END;
	...
	function check_has_mod_access($topic_id) // This is here to prevent snooping into the details of things (the backend provides the true security)
	{
		...
		if (($private_topic) && ($topic_info[0]['t_pt_from']!=get_member()) && ($topic_info[0]['t_pt_to']!=get_member()) && (!ocf_has_special_pt_access($topic_id)) && (!has_specific_permission(get_member(),'view_other_pt')))
			access_denied('I_ERROR');
