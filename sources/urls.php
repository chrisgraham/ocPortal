...
function looks_like_url($value,$lax=false)
{
	...
	if (($lax) && (substr($value,0,1)=='{')) return true;
	return (((strpos($value,'.php')!==false) || (strpos($value,'.htm')!==false) || (substr($value,0,1)=='#') || (substr($value,0,13)=='{$FIND_SCRIPT') || (substr($value,0,10)=='{$BASE_URL') || (substr(strtolower($value),0,11)=='javascript:') || (substr($value,0,7)=='http://') || (substr($value,0,3)=='../') || (substr($value,0,6)=='ftp://'))) && (strpos($value,'<')===false);
}
