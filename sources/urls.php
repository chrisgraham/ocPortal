...
function can_try_mod_rewrite($avoid_remap = false)
{
    ...
    return (($url_scheme != 'RAW') && (get_param_integer('keep_no_short_url', 0) == 1) && ((!array_key_exists('block_mod_rewrite', $GLOBALS['SITE_INFO'])) || ($GLOBALS['SITE_INFO']['block_mod_rewrite'] == '0')) && (!$avoid_remap)); // If we don't have the option on or are not using apache, return
}
...
function _build_url($vars, $zone_name = '', $skip = null, $keep_all = false, $avoid_remap = false, $skip_keep = false, $hash = '')
{
    ...
    $test_rewrite = null;
    $self_page = ((!isset($vars['page'])) || ((function_exists('get_zone_name')) && (get_zone_name() == $zone_name) && (($vars['page'] == '_SELF') || ($vars['page'] == get_param('page', ''))))) && ((!isset($vars['type'])) || ($vars['type'] == get_param('type', 'browse'))) && ($hash != '#_top') && (!$KNOWN_AJAX);
