...
function cedi_breadcrumbs($chain,$current_title=NULL,$final_link=false,$links=true,$this_link_virtual_root=false)
{
	...
	while ($token!==false)
	{
		...
		} else
		{
			...
			if ($final_link)
			{
				$insbreadcrumbs->attach(hyperlink($url,escape_html($current_title),false,false,$this_link_virtual_root?do_lang_tempcode('VIRTUAL_ROOT'):do_lang_tempcode('GO_BACKWARDS_TO',escape_html($current_title)),NULL,NULL,'up'));
			} else
			{
