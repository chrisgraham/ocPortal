		...
		switch ($name) // Order by how common (performance)
		{
			...
			case 'INSERT_SPAMMER_BLACKHOLE':
				if (get_option('spam_blackhole_detection')=='1')
				{
					...
					$value='<div id="'.escape_html($field_name).'_wrap" style="display:none"><label for="'.escape_html($field_name).'">'.do_lang('DO_NOT_FILL_ME_SPAMMER_BLACKHOLE').'</label><input autocomplete="off" id="'.escape_html($field_name).'" name="'.escape_html($field_name).'" value="" type="text" /></div>';
			...
			case 'RATING':
				if (isset($param[1]))
				{
					...
					$rating=get_rating_simple_array(array_key_exists(3,$param)?$param[3]:get_self_url(true),array_key_exists(4,$param)?$param[4]:(is_null($DISPLAYED_TITLE)?'':$DISPLAYED_TITLE->evaluate()),$param[0],$param[1],array_key_exists(5,$param)?$param[5]:'RATING_FORM',array_key_exists(2,$param)?intval($param[2]):NULL);
					if ($rating!==NULL)
					{
...
function symbol_truncator($param,$type,$tooltip_if_truncated=NULL)
{
	...
	if ((isset($not_html[$amount])/*optimisation*/) && (ocp_mb_strlen($not_html)>$amount))
	{
		...
		{
			case 'left':
				$temp=(($is_html || $grammar_completeness_tolerance!=0.0)?xhtml_substr($html,0,max($amount-3,1),$literal_pos,false,$grammar_completeness_tolerance):escape_html(ocp_mb_substr($not_html,0,max($amount-3,1))));
			...
			case 'expand':
				...
				break;
			case 'right':
				$truncated=str_replace(array('</p>&hellip;','</div>&hellip;'),array('&hellip;</p>','&hellip;</div>'),('&hellip;'.ltrim(($is_html || $grammar_completeness_tolerance!=0.0)?xhtml_substr($html,-max($amount-3,1),NULL,$literal_pos,false,$grammar_completeness_tolerance):escape_html(ocp_mb_substr($not_html,-max($amount-3,1))))));
				break;
			case 'spread':
