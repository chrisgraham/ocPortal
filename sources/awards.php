...
function get_award_fields($content_type,$id=NULL)
{
	...
	foreach ($rows as $row)
	{
		if (has_category_access(get_member(),'award',strval($row['id'])))
		{
			...
			$fields->attach(form_input_tick(get_translated_text($row['a_title']),(get_translated_text($row['a_description'])=='')?new ocp_tempcode():do_lang_tempcode('PRESENT_AWARD',get_translated_tempcode('award_types',$row,'a_description')),'award_'.strval($row['id']),$has_award));
		}
	}
