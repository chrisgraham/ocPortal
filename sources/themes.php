...
function find_theme_image($id,$silent_fail=false,$leave_local=false,$theme=NULL,$lang=NULL,$db=NULL,$pure_only=false)
{
	...
	if ((!$truism) && (!$pure_only)) // Separate lookup, cannot go through $IMG_CODES
	{
		...
		if ($path!==NULL)
		{
			...
			{
				$path=(($db!=$GLOBALS['SITE_DB'])?get_forum_base_url():((substr($path,0,22)=='themes/default/images/')?get_base_url():get_custom_base_url())).'/'.$path;
			}
	...
	if (($pure_only) || (!isset($IMG_CODES[$site][$id])) || (!$truism)/* || (($IMG_CODES[$site][$id]=='') && (!$silent_fail))*/)
	{
		...
		if ($db==$GLOBALS['SITE_DB']) // If guard is here because a MSN site can't make assumptions about the file system of the central site
		{
