...
function _symbol_thumbnail($param)
{
	...
	if (($param[0]!=''))
	{
		...
		if ((!is_file($save_path)) && (!is_file($save_path.'.png')))
		{
			...
			if (!$result) $value=(trim(isset($param[4])?$param[4]:'')=='')?$orig_url:$param[4];
		}
...
function is_video($name,$as_admin,$must_be_true_video=false)
{
	...
	$hooks=find_media_renderers($url,array(),$as_admin,NULL,$acceptable_media);
...
{
	require_code('media_renderer');
	$hooks=find_media_renderers($url,array(),$as_admin,NULL);
	return !is_null($hooks);
}
