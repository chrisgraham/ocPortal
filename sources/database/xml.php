...
class Database_Static_xml
{
					...
					foreach ($schema as $key=>$val)
					{
						if ((preg_replace('#[^\w]#','',$val)=='AUTO') && (!in_array($key,$insert_keys)))
						{
							...
							foreach (array_keys($inserts) as $i)
							{
								...
								{
									if ($GLOBALS['IN_MINIKERNEL_VERSION']==1) // In particular the f_groups/f_categories/calendar_types usage of tables references ID numbers for things. But let's just make all installer stuff linear
									{
										$inserts[$i][$key]++;
	...
	function _write_record($db,$table_name,$guid,$record,$fail_ok=false,$save_as_volatile=false)
	{
		...
		if (!defined('PHALANGER'))
			while (!flock($myfile,LOCK_EX)) sleep(1);
