...
// 		print_r(func_get_args());exit();
	...
	{
		require_code('ocf_forum_driver_helper');
		return _helper_get_forum_topic_posts($this,$forum_name,$topic_name,$topic_description,$count,$max,$start);
	}
	...
	function get_member_avatar_url($member)
	{
		...
		elseif ((url_is_local($avatar)) && ($avatar!='')) $avatar=get_complex_base_url($avatar).'/'.$avatar;
				...
				switch ($result)
				{
					...
					default:
						...
						if (array_key_exists(0,$member_rows))
						{
							...
							if (!$cookie_login)
							{
								...
								if (post_param_integer('finishing_profile',0)==1)
								{
									...
									{
										$custom_fields=$custom_fields+ocf_read_in_custom_fields(ocf_get_all_custom_fields_match(ocf_get_all_default_groups(true),NULL,NULL,NULL,NULL,NULL,NULL,false,1));
									} else
									{
						...
						} else
						{
							...
							ocf_make_member($member_row['m_username'],$password_hashed,$member_row['m_email_address'],NULL,NULL,NULL,NULL,$custom_fields,$member_row['m_timezone_offset'],NULL,1,NULL,NULL,'',$member_row['m_avatar_url'],$member_row['m_signature'],0,$member_row['m_preview_posts'],1,$member_row['m_title'],$member_row['m_photo_url'],$member_row['m_photo_thumb_url'],$member_row['m_views_signatures'],$member_row['m_track_contributed_topics'],$member_row['m_language'],1,'',NULL,'',false,'remote','',$member_row['m_zone_wide']);
