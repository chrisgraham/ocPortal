...
class forum_driver_vb_shared extends forum_driver_base
{
	...
	function get_tid_from_topic($topic,$forum)
	{
		...
		return $this->connection->query_value_null_ok_full('SELECT threadid FROM '.$this->connection->get_table_prefix().'thread WHERE forumid='.strval((integer)$fid).' AND ('.db_string_equal_to('title',$topic).' OR title LIKE \'% (#'.db_encode_like($topic).')\')');
	}
	...
	function make_post_forum_topic($forum_name,$topic_name,$member,$post,$title,$_topic_for,$time=NULL,$ip=NULL)
	{
		...
		if (is_null($tid))
		{
			$tid=$this->connection->query_insert('thread',array('title'=>$topic_name,'lastpost'=>$time,'forumid'=>$cf,'open'=>1,'postusername'=>$username,'postuserid'=>$member,'lastposter'=>$username,'dateline'=>$time,'visible'=>1),true);
			$this->connection->query_insert('post',array('threadid'=>$tid,'username'=>do_lang('SYSTEM','','','',get_site_default_lang()),'userid'=>0,'title'=>'','dateline'=>$time,'pagetext'=>do_lang('SPACER_POST',$topic_for,'','',get_site_default_lang()),'allowsmilie'=>1,'ipaddress'=>'127.0.0.1','visible'=>1));
			$this->connection->query('UPDATE '.$this->connection->get_table_prefix().'forum SET threadcount=(threadcount+1) WHERE forumid='.strval((integer)$cf),1);
		}
		if ($post=='') return;
		$last_post_id=$this->connection->query_insert('post',array('threadid'=>$tid,'username'=>$username,'userid'=>$member,'title'=>$title,'dateline'=>$time,'pagetext'=>$post,'allowsmilie'=>1,'ipaddress'=>$ip,'visible'=>1),true);
		$this->connection->query('UPDATE '.$this->connection->get_table_prefix().'forum SET replycount=(replycount+1), lastpost='.strval($time).', lastposter=\''.db_escape_string($username).'\' WHERE forumid='.strval((integer)$cf),1);
		if ((!isset($GLOBALS['SITE_INFO']['vb_version'])) || ($GLOBALS['SITE_INFO']['vb_version']>=3.6))
		{
			$this->connection->query('UPDATE '.$this->connection->get_table_prefix().'thread SET lastpostid='.strval((integer)$last_post_id).', replycount=(replycount+1), lastpost='.strval($time).', lastposter=\''.db_escape_string($username).'\' WHERE threadid='.strval((integer)$tid),1);
		} else
		{
			$this->connection->query('UPDATE '.$this->connection->get_table_prefix().'thread SET replycount=(replycount+1), lastpost='.strval($time).', lastposter=\''.db_escape_string($username).'\' WHERE threadid='.strval((integer)$tid),1);
		}
	}
	...
	function get_forum_topic_posts($forum_name,$topic_name,$topic_description,&$count,$max=100,$start=0)
	{
		...
		$rows=$this->connection->query('SELECT * FROM '.$this->connection->get_table_prefix().'post WHERE threadid='.strval((integer)$topic_id).' AND pagetext NOT LIKE \''.db_encode_like(substr(do_lang('SPACER_POST','','','',get_site_default_lang()),0,20).'%').'\' ORDER BY dateline',$max,$start);
	...
	function show_forum_topics($name,$limit,$start,&$max_rows,$filter_topic_name='',$show_first_posts=false,$date_key='lasttime',$hot=false,$filter_topic_description='')
	{
		...
		$topic_filter=($filter_topic_name!='')?('AND title LIKE \''.db_encode_like($filter_topic_name).'\''):'';
