...
class forum_driver_phpbb2 extends forum_driver_base
{
	...
	function make_post_forum_topic($forum_name,$topic_name,$member,$_postdetails,$title,$_topic_for,$time=NULL,$ip=NULL)
	{
		...
		if (is_null($tid))
		{
			$map=array('forum_id'=>$cf,'topic_title'=>$topic_name,'topic_poster'=>$member,'topic_time'=>$time,'topic_views'=>0,'topic_replies'=>0,'topic_status'=>0,'topic_vote'=>0,'topic_type'=>0,'topic_first_post_id'=>0,'topic_last_post_id'=>0,'topic_moved_id'=>0);
			...
			$map=array('topic_id'=>$tid,'forum_id'=>$cf,'poster_id'=>-1,'post_time'=>$time,'poster_ip'=>$local_ip_address,'post_username'=>NULL,'enable_bbcode'=>1,'enable_html'=>0,'enable_smilies'=>1,'enable_sig'=>1,'post_edit_time'=>NULL,'post_edit_count'=>0);
			...
			$map=array('post_id'=>$pid,'bbcode_uid'=>'','post_subject'=>'','post_text'=>do_lang('SPACER_POST',$topic_for,'','',get_site_default_lang()));
		...
		if ($post=='') return;
		$map=array('topic_id'=>$tid,'forum_id'=>$cf,'poster_id'=>$member,'post_time'=>$time,'poster_ip'=>$ip_address,'post_username'=>NULL,'enable_bbcode'=>1,'enable_html'=>0,'enable_smilies'=>1,'enable_sig'=>1,'post_edit_time'=>NULL,'post_edit_count'=>0);
		if ($fm) $map=array_merge($map,array('post_attachment'=>0,'post_edit_user'=>NULL,'post_icon'=>0,'post_bluecard'=>NULL,'rating_rank_id'=>0,'user_avatar'=>NULL,'user_avatar_type'=>0,'urgent_post'=>0));
		$pid=$this->connection->query_insert('posts',$map,true);
		$map=array('post_id'=>$pid,'bbcode_uid'=>'','post_subject'=>$title,'post_text'=>$post);
		$this->connection->query_insert('posts_text',$map);
		...
		$this->connection->query('UPDATE '.$this->connection->get_table_prefix().'topics SET topic_replies=(topic_replies+1), topic_last_post_id='.strval((integer)$pid).' WHERE topic_id='.strval((integer)$tid),1);
	}
