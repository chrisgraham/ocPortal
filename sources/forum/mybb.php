...
class forum_driver_mybb extends forum_driver_base
{
	 ...
	 */
	function make_post_forum_topic($forum_name,$topic_name,$member,$_post,$title,$_topic_for,$time=NULL,$ip=NULL)
	{
		...
		if (is_null($tid))
		{
			$tid=$this->connection->query_insert('threads',array('fid'=>$cf,'subject'=>$topic_name,'username'=>$username,'uid'=>$member,'lastposter'=>$username,'lastposteruid'=>$member,'visible'=>1,'dateline'=>$time,'lastpost'=>$time),true);

			$this->connection->query_insert('posts',array('fid'=>$cf,'tid'=>$tid,'username'=>do_lang('SYSTEM','','','',get_site_default_lang()),'uid'=>$member,'message'=>do_lang('SPACER_POST',$topic_for,'','',get_site_default_lang()),'subject'=>'','dateline'=>$time,'visible'=>1,'ipaddress'=>$ip_address,'longipaddress'=>$long_ip_address));

			$this->connection->query('UPDATE '.$this->connection->get_table_prefix().'forums SET posts=(posts+1), threads=(threads+1) WHERE fid='.(string)intval($cf),1);
		}
		if ($post=='') return;
		

		$this->connection->query_insert('posts',array('fid'=>$cf,'tid'=>$tid,'username'=>$username,'uid'=>$member,'message'=>$post,'subject'=>$title,'dateline'=>$time,'visible'=>1,'ipaddress'=>$ip_address,'longipaddress'=>$long_ip_address));

		$this->connection->query('UPDATE '.$this->connection->get_table_prefix().'forums SET posts=(posts+1), lastpost='.strval($time).', lastposter=\''.db_escape_string($username).'\' WHERE fid='.(string)intval($cf),1);

		$this->connection->query('UPDATE '.$this->connection->get_table_prefix().'threads SET lastpost='.strval($time).', lastposter=\''.db_escape_string($username).'\', lastposteruid='.strval($member).', replies=(replies+1) WHERE tid='.(string)intval($tid),1);
	}
	...
	function get_forum_topic_posts($forum_name,$topic_name,$topic_description,&$count,$max=100,$start=0)
	{
		...
		$rows=$this->connection->query('SELECT * FROM '.$this->connection->get_table_prefix().'posts WHERE tid='.(string)intval($topic_id).' AND message NOT LIKE \''.db_encode_like(substr(do_lang('SPACER_POST','','','',get_site_default_lang()),0,20).'%').'\' ORDER BY dateline',$max,$start);
	...
	function get_tid_from_topic($topic,$forum)
	{
		...
		return $this->connection->query_value_null_ok_full('SELECT tid FROM '.$this->connection->get_table_prefix().'threads WHERE fid='.(string)intval($fid).' AND ('.db_string_equal_to('subject',$topic).' OR subject LIKE \'% (#'.db_encode_like($topic).')\')');
	}
