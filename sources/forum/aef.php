...
class forum_driver_aef extends forum_driver_base
{
	...
	function get_forum_topic_posts($forum_name,$topic_name,$topic_description,&$count,$max=100,$start=0)
	{
		...
		$rows=$this->connection->query('SELECT * FROM '.$this->connection->get_table_prefix().'posts p WHERE post_tid='.strval((integer)$topic_id).' AND post NOT LIKE \''.db_encode_like(substr(do_lang('SPACER_POST','','','',get_site_default_lang()),0,20).'%').'\' ORDER BY ptime',$max,$start);
