...
class forum_driver_phpbb3 extends forum_driver_base
{
	 ...
	 */
	function make_post_forum_topic($forum_name,$topic_name,$member,$_postdetails,$title,$_topic_for,$time=NULL,$ip=NULL)
	{
		...
		if (is_null($tid))
		{
			$map=array('forum_id'=>$cf,'topic_title'=>$topic_name,'topic_poster'=>$member,'topic_time'=>$time,'topic_views'=>0,'topic_replies'=>0,'topic_replies_real'=>0,'topic_status'=>0,'topic_type'=>0,'topic_first_post_id'=>0,'topic_last_post_id'=>0,'topic_moved_id'=>0);
			...
			$map=array('topic_id'=>$tid,'forum_id'=>$cf,'poster_id'=>-1,'post_text'=>do_lang('SPACER_POST',$topic_for,'','',get_site_default_lang()),'post_time'=>$time,'poster_ip'=>$local_ip_address,'post_username'=>$this->get_username($member),'enable_bbcode'=>1,'enable_smilies'=>1,'enable_sig'=>1,'post_edit_time'=>0,'post_edit_count'=>0);
			...
			$this->connection->query('UPDATE '.$this->connection->get_table_prefix().'topics SET topic_first_post_id='.strval((integer)$pid).' WHERE topic_id='.strval((integer)$tid),1);
			$this->connection->query('UPDATE '.$this->connection->get_table_prefix().'forums SET forum_topics=(forum_topics+1),forum_posts=(forum_posts+1) WHERE forum_id='.strval((integer)$cf),1);
		}
		if ($post=='') return;
		$map=array('topic_id'=>$tid,'forum_id'=>$cf,'poster_id'=>$member,'post_text'=>$post,'post_time'=>$time,'poster_ip'=>$ip_address,'post_username'=>$this->get_username($member),'enable_bbcode'=>1,'enable_smilies'=>1,'enable_sig'=>1,'post_edit_time'=>0,'post_edit_count'=>0);
		if ($fm) $map=array_merge($map,array('post_attachment'=>0,'post_edit_user'=>NULL,'post_icon'=>0,'post_bluecard'=>NULL,'rating_rank_id'=>0,'user_avatar'=>NULL,'user_avatar_type'=>0,'urgent_post'=>0));
		$pid=$this->connection->query_insert('posts',$map,true);
		$this->connection->query('UPDATE '.$this->connection->get_table_prefix().'forums SET forum_posts=(forum_posts+1), forum_last_post_id='.strval((integer)$pid).' WHERE forum_id='.strval((integer)$cf),1);
		$this->connection->query('UPDATE '.$this->connection->get_table_prefix().'topics SET topic_replies=(topic_replies+1), topic_replies_real=(topic_replies_real+1), topic_last_post_id='.strval((integer)$pid).' WHERE topic_id='.strval((integer)$tid),1);
	}
	...
	function get_forum_topic_posts($forum_name,$topic_name,$topic_description,&$count,$max=100,$start=0)
	{
		...
		$rows=$this->connection->query('SELECT * FROM '.$this->connection->get_table_prefix().'posts p WHERE topic_id='.strval((integer)$topic_id).' AND post_text NOT LIKE \''.db_encode_like(substr(do_lang('SPACER_POST','','','',get_site_default_lang()),0,20).'%').'\' ORDER BY post_time',$max,$start);
	...
	function get_tid_from_topic($topic,$forum)
	{
		...
		return $this->connection->query_value_null_ok_full('SELECT topic_id FROM '.$this->connection->get_table_prefix().'topics WHERE forum_id='.strval((integer)$fid).' AND ('.db_string_equal_to('topic_title',$topic).' OR topic_title LIKE \'% (#'.db_encode_like($topic).')\')');
	}
	...
	function show_forum_topics($name,$limit,$start,&$max_rows,$filter_topic_name='',$show_first_posts=false,$date_key='lasttime',$hot=false,$filter_topic_description='')
	{
		...
		$topic_filter=($filter_topic_name!='')?'AND topic_title LIKE \''.db_encode_like($filter_topic_name).'\'':'';
