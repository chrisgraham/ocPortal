...
//		$post=$__post->evaluate();
		...
		if (is_null($tid))
		{
			$tid=$this->connection->query_insert('topics',array('id_board'=>$cf,'id_first_msg'=>mt_rand(0,100000),'id_last_msg'=>mt_rand(0,100000),'id_member_started'=>$member,'id_member_updated'=>$member,'num_replies'=>2),true);
			$pid=$this->connection->query_insert('messages',array('id_topic'=>$tid,'id_board'=>$cf,'poster_time'=>$time,'id_member'=>$this->get_guest_id(),'subject'=>$topic_name,'poster_name'=>do_lang('SYSTEM','','','',get_site_default_lang()),'poster_email'=>get_option('staff_address'),'poster_ip'=>'127.0.0.1','modified_name'=>'','body'=>do_lang('SPACER_POST',$topic_for,'','',get_site_default_lang())),true);
	
			$this->connection->query('UPDATE '.$this->connection->get_table_prefix().'boards SET num_posts=(num_posts+1), num_topics=(num_topics+1) WHERE id_board='.strval((integer)$cf),1);
		...
		$pid=$this->connection->query_insert('messages',array('id_topic'=>$tid,'id_board'=>$cf,'poster_time'=>$time,'id_member'=>$member,'subject'=>$title,'poster_name'=>$username,'poster_email'=>$email,'poster_ip'=>$ip,'body'=>$post,'modified_name'=>''),true);
		$this->connection->query('UPDATE '.$this->connection->get_table_prefix().'boards SET id_last_msg='.strval((integer)$pid).', num_posts=(num_posts+1) WHERE id_board='.strval((integer)$cf),1); // , last_updated='.strval($time).'
		$this->connection->query('UPDATE '.$this->connection->get_table_prefix().'topics SET id_last_msg='.strval((integer)$pid).', num_replies=(num_replies+1), id_member_updated='.strval((integer)$member).' WHERE id_topic='.strval((integer)$tid),1);
	}
	...
	function get_forum_topic_posts($forum_name,$topic_name,$topic_description,&$count,$max=100,$start=0)
	{
		...
		$rows=$this->connection->query('SELECT * FROM '.$this->connection->get_table_prefix().'messages WHERE id_topic='.strval((integer)$topic_id).' AND body NOT LIKE \''.db_encode_like(substr(do_lang('SPACER_POST','','','',get_site_default_lang()),0,20).'%').'\' ORDER BY poster_time',$max,$start);
