			...
			} else
			{
				...
				if (!is_guest($p['member_id']))
				{
					...
					if ($left_room_msg!='')
					{
						...
						$_message_parsed=insert_lang_comcode('the_message',$left_room_msg,4);
						$message_id=$GLOBALS['SITE_DB']->query_insert('chat_messages',array('system_message'=>1,'ip_address'=>get_ip_address(),'room_id'=>$p['room_id'],'user_id'=>$p['member_id'],'date_and_time'=>time(),'text_colour'=>get_option('chat_default_post_colour'),'font_name'=>get_option('chat_default_post_font'))+$_message_parsed,true);
...
function chat_post_message($room_id,$message,$font_name,$text_colour,$wrap_pos=60)
{
	...
	if ((is_null($time_last_message)) || ($time_left<=0))
	{
		...
		$_message_parsed=insert_lang_comcode('the_message',wordfilter_text($message),4,NULL,false,NULL,$wrap_pos);
		$message_id=$GLOBALS['SITE_DB']->query_insert('chat_messages',array('system_message'=>0,'ip_address'=>get_ip_address(),'room_id'=>$room_id,'user_id'=>get_member(),'date_and_time'=>time(),'text_colour'=>$text_colour,'font_name'=>$font_name)+$_message_parsed,true);
		...
		foreach (array_keys($hooks) as $hook)
		{
			...
			if ((!is_null($ob)) && (method_exists($ob,'reply_to_any_communication')))
			{
				...
				{
					// Store bots message
					$_message_parsed=insert_lang_comcode('the_message',wordfilter_text($response),4,NULL,false,NULL,$wrap_pos);
					$bot_message_id=$GLOBALS['SITE_DB']->query_insert('chat_messages',array('system_message'=>0,'ip_address'=>$hook,'room_id'=>$room_id,'user_id'=>$GLOBALS['FORUM_DRIVER']->get_guest_id(),'date_and_time'=>time(),'text_colour'=>get_option('chat_default_post_colour'),'font_name'=>get_option('chat_default_post_font'))+$_message_parsed,true);
	...
	require_lang('chat');
	$_message_parsed=insert_lang_comcode('the_message','[private="'.$GLOBALS['FORUM_DRIVER']->get_username(get_member()).'"]'.do_lang('FLOOD_CONTROL_BLOCKED',integer_format($time_left)).'[/private]',4,NULL,false,NULL/*,$wrap_pos*/); // Can't wrap system messages, the Comcode parser won't know 'private' is a real tag so will wrap inside it's definition
	$message_id=$GLOBALS['SITE_DB']->query_insert('chat_messages',array('system_message'=>1,'ip_address'=>get_ip_address(),'room_id'=>$room_id,'user_id'=>get_member(),'date_and_time'=>time(),'text_colour'=>get_option('chat_default_post_colour'),'font_name'=>get_option('chat_default_post_font'))+$_message_parsed,true);
	$myfile=@fopen(get_custom_file_base().'/data_custom/modules/chat/chat_last_msg.dat','wb') OR intelligent_write_error(get_custom_file_base().'/data_custom/modules/chat/chat_last_msg.dat');
	fwrite($myfile,strval($message_id));
	fclose($myfile);
	sync_file(get_custom_file_base().'/data_custom/modules/chat/chat_last_msg.dat');
...
function chat_get_room_content($room_id,$_rooms,$cutoff=NULL,$dereference=false,$downloading=false,$start=NULL,$finish=NULL,$uptoid=NULL,$zone=NULL,$entering_room=NULL,$return_my_messages=true,$return_system_messages=true)
{
	...
	if (!is_null($entering_room))
	{
		...
		{
			require_code('comcode');
			$_message_parsed=insert_lang_comcode('the_message','[private="'.$their_username.'"]'.$_entering_room.'[/private]',4);
			$message_id=$GLOBALS['SITE_DB']->query_insert('chat_messages',array('system_message'=>0,'ip_address'=>get_ip_address(),'room_id'=>$room_id,'user_id'=>get_member(),'date_and_time'=>time(),'text_colour'=>get_option('chat_default_post_colour'),'font_name'=>get_option('chat_default_post_font'))+$_message_parsed,true);
			$myfile=@fopen(get_custom_file_base().'/data_custom/modules/chat/chat_last_msg.dat','wb') OR intelligent_write_error(get_custom_file_base().'/data_custom/modules/chat/chat_last_msg.dat');
			fwrite($myfile,strval($message_id));
			fclose($myfile);
			sync_file(get_custom_file_base().'/data_custom/modules/chat/chat_last_msg.dat');
		...
		{
			require_code('comcode');
			$_message_parsed=insert_lang_comcode('the_message',$enter_room_msg,4);
			$message_id=$GLOBALS['SITE_DB']->query_insert('chat_messages',array('system_message'=>1,'ip_address'=>get_ip_address(),'room_id'=>$room_id,'user_id'=>get_member(),'date_and_time'=>time(),'text_colour'=>get_option('chat_default_post_colour'),'font_name'=>get_option('chat_default_post_font'))+$_message_parsed,true);
			$myfile=@fopen(get_custom_file_base().'/data_custom/modules/chat/chat_last_msg.dat','wb') OR intelligent_write_error(get_custom_file_base().'/data_custom/modules/chat/chat_last_msg.dat');
			fwrite($myfile,strval($message_id));
			fclose($myfile);
		...
		foreach ($_pm_message as $person)
		{
			if (($person!=$GLOBALS['FORUM_DRIVER']->get_username(get_member())) && ($person!=do_lang('GUEST')))
			{
				foreach ($rooms as $room)
				{
					...
					{
						$_message_parsed=insert_lang_comcode('the_message','[invite="'.$person.'"]'.get_chatroom_name($new_room_id).'[/invite]',4);
						$message_id=$GLOBALS['SITE_DB']->query_insert('chat_messages',array('ip_address'=>get_ip_address(),'room_id'=>$room['id'],'user_id'=>get_member(),'date_and_time'=>time(),'text_colour'=>get_option('chat_default_post_colour'),'font_name'=>get_option('chat_default_post_font'))+$_message_parsed,true);
						$myfile=@fopen(get_custom_file_base().'/data_custom/modules/chat/chat_last_msg.dat','wb') OR intelligent_write_error(get_custom_file_base().'/data_custom/modules/chat/chat_last_msg.dat');
						fwrite($myfile,strval($message_id));
						fclose($myfile);
						sync_file(get_custom_file_base().'/data_custom/modules/chat/chat_last_msg.dat');
