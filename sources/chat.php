...
function chat_logs_script()
{
	...
	$messages=chat_get_room_content($room,$room_check,NULL,false,true,intval($start),intval($finish),NULL,get_param('zone',get_module_zone('chat')));
...
function chat_get_room_content($room_id,$_rooms,$cutoff=NULL,$dereference=false,$downloading=false,$start=NULL,$finish=NULL,$uptoid=NULL,$zone=NULL,$entering_room=NULL,$return_my_messages=true,$return_system_messages=true)
{
	...
	foreach (array_keys($rows) as $i)
	{
		...
		if ($room_id==-1)
		{
			$pm_message_deleted=(!array_key_exists($rows[$i]['room_id'],$rooms)) || (($rooms[$rows[$i]['room_id']]['is_im']==0) && (!check_chatroom_access($rooms[$rows[$i]['room_id']],true))) || (($rooms[$rows[$i]['room_id']]['is_im']==1) && (!check_chatroom_access($rooms[$rows[$i]['room_id']],true,null,true)));
		} else
		{
