 ...
 */
function init__templates_results_browser()
{
...
function results_browser($title,$category_id,$start,$start_name,$max,$max_name,$max_rows,$root=NULL,$type=NULL,$keep_all=false,$keep_post=false,$max_page_links=7,$_selectors=NULL)
{
	...
	if ($max<$max_rows) // If they don't all fit on one page
	{
		...
		$hidden=build_keep_form_fields('_SELF',true,array($max_name,'wide_high'));
		...
		if ($from>0) $part->attach(do_template('RESULTS_BROWSER_CONTINUE'));
		...
		// Indicate that the sequence is incomplete with an ellipsis
		if ($to<$num_pages) $part->attach(do_template('RESULTS_BROWSER_CONTINUE'));
		...
		} else $part->attach(do_template('RESULTS_BROWSER_NEXT'));
...
function _build_results_browser_cat_url($url_array,$post_array,$type,$root,$category_id,$keep_all)
{
	...
	if (strpos(ocp_srv('REQUEST_URI'),'/iframe.php')!==false)
	{
		...
		if ($keep_all) $url_array=array_merge($_GET,$_POST,$url_array);
		foreach ($url_array as $key=>$param)
		{
			...
			if ($key!='zone') $cat_url->attach('&'.$key.'='.urlencode($param));
		}
	} else
	{
