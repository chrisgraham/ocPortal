	...
	if ((get_forum_type()=='ocf') && (addon_installed('ecommerce')))
	{
		...
		foreach ($_subscriptions as $sub)
		{
			...
			if (substr($sub['s_type_code'],0,9)=='USERGROUP')
			{
				...
				if (!array_key_exists($usergroup_subscription_id,$usergroup_subscription_rows)) continue;
				...
				$usergroup_subscription_description=get_translated_text($usergroup_subscription_row['s_description']);
				...
				$item_name=$usergroup_name;
			} else
			{
				if ($usergroup_subscriptions_only) continue;
			...
			if (!$is_manual)
			{
				if ($sub['s_state']!='cancelled') $expiry_time=NULL; // Auto-renewal --> no expiry
			}
