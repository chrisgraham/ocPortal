...
function handle_conflict_resolution($id=NULL,$only_staff=false)
{
	...
	require_javascript('javascript_ajax');
	$last_edit_screen_time=$GLOBALS['SITE_DB']->query('SELECT * FROM '.$GLOBALS['SITE_DB']->get_table_prefix().'edit_pings WHERE '.db_string_equal_to('the_page',substr(get_page_name(),0,80)).' AND '.db_string_equal_to('the_type',substr(get_param('type','misc'),0,80)).' AND '.db_string_equal_to('the_id',substr($id,0,80)).' AND the_member<>'.strval((integer)get_member()).' ORDER BY the_time DESC',1);
