...
class Hook_sitemap_quiz extends Hook_sitemap_content
{
	protected $content_type='quiz';
	protected $screen_type='do';
	...
	function get_virtual_nodes($pagelink,$callback=NULL,$valid_node_types=NULL,$child_cutoff=NULL,$max_recurse_depth=NULL,$recurse_level=0,$require_permission_support=false,$zone='_SEARCH',$use_page_groupings=false,$consider_secondary_categories=false,$consider_validation=false,$meta_gather=0,$return_anyway=false)
	{
		...
		{
			$count=$GLOBALS['SITE_DB']->query_select_value('quizzes','COUNT(*)',$consider_validation?array('q_validated'=>1):array());
		...
		do
		{
			$rows=$GLOBALS['SITE_DB']->query_select('quizzes',array('*'),$consider_validation?array('q_validated'=>1):array(),'',SITEMAP_MAX_ROWS_PER_LOOP,$start);
			foreach ($rows as $row)
			{
