...
class Hook_sitemap_news_category extends Hook_sitemap_content
{
	...
	function get_virtual_nodes($page_link,$callback=NULL,$valid_node_types=NULL,$child_cutoff=NULL,$max_recurse_depth=NULL,$recurse_level=0,$require_permission_support=false,$zone='_SEARCH',$use_page_groupings=false,$consider_secondary_categories=false,$consider_validation=false,$meta_gather=0,$return_anyway=false)
	{
		...
		do
		{
			...
			foreach ($rows as $row)
			{
				...
				if (strpos($page_link,':blog=0')!==false) $child_page_link.=':blog=0';
				if (strpos($page_link,':blog=1')!==false) $child_page_link.=':blog=1';
	...
	function get_node($page_link,$callback=NULL,$valid_node_types=NULL,$child_cutoff=NULL,$max_recurse_depth=NULL,$recurse_level=0,$require_permission_support=false,$zone='_SEARCH',$use_page_groupings=false,$consider_secondary_categories=false,$consider_validation=false,$meta_gather=0,$row=NULL,$return_anyway=false)
	{
		...
		if (!is_null($children))
		{
			...
			{
				if (strpos($page_link,':blog=0')!==false) $child['page_link'].=':blog=0';
				if (strpos($page_link,':blog=1')!==false) $child['page_link'].=':blog=1';
			}
			if ($consider_secondary_categories)
			{
				...
				if (!$skip_children)
				{
					...
					foreach ($child_rows as $child_row)
					{
						...
						$child_node=$child_hook_ob->get_node($child_page_link,$callback,$valid_node_types,$child_cutoff,$max_recurse_depth,$recurse_level+1,$require_permission_support,$zone,$use_page_groupings,$consider_secondary_categories,$consider_validation,$meta_gather);
						if ($child_node!==NULL)
							$children[]=$child_node;
					}
				}
