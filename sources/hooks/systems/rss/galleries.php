...
class Hook_rss_galleries
{
	...
	function run($_filters,$cutoff,$prefix,$date_string,$max)
	{
		...
		$rows1=$GLOBALS['SITE_DB']->query('SELECT r.* FROM '.$GLOBALS['SITE_DB']->get_table_prefix().'videos r'.$privacy_join.' WHERE add_date>'.strval($cutoff).' AND '.$filters.(((!has_specific_permission(get_member(),'see_unvalidated'))))?' AND validated=1 ':'').$privacy_where.' ORDER BY add_date DESC',$max);
		...
		$rows2=$GLOBALS['SITE_DB']->query('SELECT r.* FROM '.$GLOBALS['SITE_DB']->get_table_prefix().'images r'.$privacy_join.' WHERE add_date>'.strval($cutoff).' AND '.$filters.(((!has_specific_permission(get_member(),'see_unvalidated'))))?' AND validated=1 ':'').$privacy_where.' ORDER BY add_date DESC',$max);
