...
class Hook_rss_wiki
{
	...
	{
		if (!addon_installed('wiki')) return NULL;
		...
		$query.=' OR EXISTS(SELECT * FROM '.$GLOBALS['SITE_DB']->get_table_prefix().'wiki_posts o WHERE o.page_id=p.id WHERE date_and_time>'.strval($cutoff).')';
		$query.=')';
		...
		foreach ($rows as $row)
		{
			...
			if (!has_category_access(get_member(),'wiki_page',strval($id))) continue;
