...
class Hook_realtime_rain_calendar
{
	...
	function run($from,$to)
	{
		...
		if (has_actual_page_access(get_member(),'calendar'))
		{
			...
			$rows=calendar_matches(get_member(),true,$from,$to); // NOTE: We also show (automatically) any RSS items the user has overlayed onto the calendar
			...
			foreach ($rows as $row)
			{
				...
				if ($timestamp[2]<$timestamp) continue;
