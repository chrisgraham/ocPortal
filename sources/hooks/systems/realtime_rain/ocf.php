	...
	function run($from,$to)
	{
		...
		if (get_forum_type()=='ocf')
		{
			...
			if ((has_actual_page_access(get_member(),'topicview')) && (addon_installed('ocf_forum')))
			{
				...
				foreach ($rows as $row)
				{
					if (is_null($row['p_cache_forum_id'])) continue;
					if (!is_null($row['p_intended_solely_for'])) continue;
					if (!has_category_access(get_member(),'forums',strval($row['p_cache_forum_id']))) continue;

					$timestamp=$row['timestamp'];
					$member_id=$row['member_id'];

					$ticker_text=strip_comcode(get_translated_text($row['p_post'],$GLOBALS['SITE_DB']));

					$drops[]=rain_get_special_icons($row['p_ip_address'],$timestamp,NULL,$ticker_text)+array(
						'TYPE'=>'post',
						'FROM_MEMBER_ID'=>strval($member_id),
						'TO_MEMBER_ID'=>NULL,
						'TITLE'=>($row['p_title']=='')?rain_truncate_for_title(strip_comcode(get_translated_text($row['p_post'],$GLOBALS['SITE_DB']))):$row['p_title'],
