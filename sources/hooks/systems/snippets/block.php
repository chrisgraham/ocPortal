	...
	function run()
	{
		...
		if (mt_rand(0,1000)==123)
		{
			if (!$GLOBALS['SITE_DB']->table_is_locked('temp_block_permissions'))
				$GLOBALS['SITE_DB']->query('DELETE FROM '.get_table_prefix().'temp_block_permissions WHERE p_time<'.strval(intval(60.0*60.0*floatval(get_option('session_expiry_time')))));
		}
