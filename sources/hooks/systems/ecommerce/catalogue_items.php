...
class Hook_catalogue_items
{
	...
	function get_available_quantity($type_code)
	{
		...
		if ($fields[3]['effective_value']!='')
		{
			...
			$query='SELECT sum(t2.p_quantity) as qty FROM '.get_table_prefix().'shopping_order t1 JOIN '.get_table_prefix().'shopping_order_details t2 ON t1.id=t2.order_id WHERE add_date>'.strval(time()-60*60*24).' AND '.db_string_equal_to('t1.order_status','ORDER_STATUS_awaiting_payment').' AND t2.p_id='.strval(intval($type_code));
			if (is_guest())
			{
