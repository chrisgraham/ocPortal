...
class Hook_cron_ip_address_sharing
{
	...
	function run()
	{
		...
		if (is_ocf_satellite_site()) return;
		if (!db_has_subqueries($GLOBALS['SITE_DB']->connection_write)) return;
		...
		if ($current_time>$time+($days*24*60*60))
		{
			...
			if (count($groups)>0)
			{
				...
				{
					if ($group_or_list_1!='') $group_or_list_1.=' OR ';
					$group_or_list_1.='m_primary_group='.strval($group);

					if ($group_or_list_2!='') $group_or_list_2.=' OR ';
