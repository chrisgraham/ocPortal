...
class Hook_main_custom_gfx_rollover_button
{
	...
	function run($map,&$block)
	{
		...
		if (!array_key_exists('img1',$map)) $map['img1']='button1';
		$img_path_1=find_theme_image($map['img1'],true,true);
		if ($img_path_1=='') return do_lang_tempcode('NO_SUCH_THEME_IMAGE',$map['img1']);

		$cache_id_1='rollover1_'.md5(serialize($map));
		$url_1=$block->_do_image($cache_id_1,$map,$img_path_1);
		if (is_object($url_1)) return $url_1;

		if (!array_key_exists('img2',$map)) $map['img2']='button2';
		$img_path_2=find_theme_image($map['img2'],true,true);
		if ($img_path_2=='') return do_lang_tempcode('NO_SUCH_THEME_IMAGE',$map['img2']);
