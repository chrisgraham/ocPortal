...
class Hook_checklist_polls
{
	...
	function run()
	{
		...
		if (get_option('poll_update_time')=='') return array();

		require_lang('polls');

		$date=$GLOBALS['SITE_DB']->query_value_null_ok('poll','date_and_time',array('is_current'=>1));
		...
		$config_row=$GLOBALS['SITE_DB']->query_select('config',array('the_page','section'),array('the_name'=>'poll_update_time'),'',1);
