...
class Hook_whats_news_news
{
	...
	function run($cutoff_time,$lang,$filter,$in_full=1)
	{
		...
		$rows=$GLOBALS['SITE_DB']->query('SELECT title,news,news_article,id,date_and_time,submitter,news_image FROM '.get_table_prefix().'news LEFT JOIN '.get_table_prefix().'news_category_entries ON news_entry=id WHERE validated=1 AND date_and_time>'.strval($cutoff_time).' AND (('.$or_list.') OR ('.$or_list_2.')) ORDER BY date_and_time DESC',$max);
