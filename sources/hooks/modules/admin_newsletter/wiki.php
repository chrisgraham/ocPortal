...
class Hook_whats_news_wiki
{
	...
	function run($cutoff_time,$lang,$filter)
	{
		...
		$rows=$GLOBALS['SITE_DB']->query('SELECT * FROM '.$GLOBALS['SITE_DB']->get_table_prefix().'wiki_pages WHERE add_date>'.strval((integer)$cutoff_time).' ORDER BY add_date DESC',300);
