...
class Hook_ipb_base
{
	...
	function import_ocf_groups($db,$table_prefix,$file_base)
	{
		...
		foreach ($rows as $row)
		{
			...
			// Page permissions
			$denies=array();
			if ($row['g_avatar_upload']==0) $denies[]=array('editavatar',get_module_zone('editavatar'));
			if ($row['g_use_search']==0) $denies[]=array('search',get_module_zone('search'));
			if ($row['g_edit_profile']==0) $denies[]=array('editprofile',get_module_zone('editprofile'));
	...
	function import_calendar($db,$table_prefix,$file_base)
	{
		...
		foreach ($rows as $row)
		{
			...
			ocf_over_msn();
			//$id_new=add_calendar_event('','','',db_get_first_id()+1,$recurrence,$recurrences,0,$event_title,$event_text,3,1-$private_event,$start_year,$start_month,$start_day,0,0,$end_year,$end_month,$end_day,0,0,$submitter); //old code

			$id_new=add_calendar_event('','','',db_get_first_id()+1,$recurrence,$recurrences,0,$event_title,$event_text,3,1-$private_event,$start_year,$start_month,$start_day,0,0,$end_year,$end_month,$end_day,NULL,NULL,1,1,1,1,'',$submitter);
	...
	function import_ocf_members($db,$table_prefix,$file_base)
	{
		...
		do
		{
			...
			foreach ($rows as $row)
			{
				...
				$id_new=ocf_make_member($row['name'],$password,$row['email'],NULL,$row['bday_day'],$row['bday_month'],$row['bday_year'],$custom_fields,strval($row['time_offset']),$primary_group,$validated,$row['joined'],$row['last_visit'],'','',$signature,0,1,1,$row['title'],'','',$row['view_sigs'],$row['auto_track'],$language,$row['email_pm'],$notes,$row['ip_address'],'',false,$type,$salt);
