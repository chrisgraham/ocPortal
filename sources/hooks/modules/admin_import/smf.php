...
class Hook_smf
{
	...
	function import_ocf_tracking($db,$table_prefix,$file_base)
	{
		...
		do
		{
			...
			foreach ($rows as $row)
			{
				if (import_check_if_imported('topic_tracking',strval($row['ID_TOPIC']).'-'.strval($row['ID_MEMBER']))) continue;
				...
				import_id_remap_put('topic_tracking',strval($row['ID_TOPIC']).'-'.strval($row['ID_MEMBER']),1);
			}
	...
	function import_calendar($db,$table_prefix,$file_base)
	{
		...
		foreach ($rows as $row)
		{
			...
			ocf_over_msn();
			$id_new=add_calendar_event('','','',db_get_first_id()+1,$recurrence,$recurrences,0,$row['title'],$description,3,1,$start_year,$start_month,$start_day,$start_hour,$start_minute,$end_year,$end_month,$end_day,$end_hour,$end_minute,1,1,1,1,'',$submitter);
			ocf_over_local();
		...
		foreach ($rows as $row)
		{
			...
			ocf_over_msn();
			$id_new=add_calendar_event('','','',db_get_first_id()+1,$recurrence,$recurrences,0,$row['title'],$row['title'],3,1,$start_year,$start_month,$start_day,$start_hour,$start_minute,$end_year,$end_month,$end_day,$end_hour,$end_minute,1,1,1,1,'',$submitter);
			ocf_over_local();
