...
class Hook_wowbb
{
	...
	function import_ocf_tracking($db,$table_prefix,$file_base)
	{
		...
		do
		{
			...
			foreach ($rows as $row)
			{
				if (import_check_if_imported('forum_tracking',strval($row['user_id']))) continue;
				...
				import_id_remap_put('forum_tracking',strval($row['user_id']),1);
			}
	...
	function import_calendar($db,$table_prefix,$file_base)
	{
		...
		foreach ($rows as $row)
		{
			...
			$id_new=add_calendar_event('','','',db_get_first_id()+1,$recurrence,intval(floor($recurrences)),0,$row['event_title'],$row['event_note'],3,$row['event_public'],$start_year,$start_month,$start_day,$start_hour,$start_minute,$end_year,$end_month,$end_day,$end_hour,$end_minute,$submitter,0,strtotime($row['event_start']));
