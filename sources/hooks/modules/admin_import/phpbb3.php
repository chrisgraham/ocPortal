...
class Hook_phpbb3
{
        ...
        do {
            ...
            foreach ($rows as $row) {
                ...
                if ($row['post_username'] == '') {
                    $row['post_username'] = $GLOBALS['OCF_DRIVER']->get_username($member_id);
                }
    ...
    public function import_reported_posts_forum($db,$table_prefix,$file_base)
    {
        ...
        foreach ($rows as $row) {
            ...
            $post = do_template('OCF_REPORTED_POST',array('_GUID' => '0dd1532216390f75385323ce11baedba','POST_ID' => strval($post_id),'MEMBER' => $member,'TOPIC_NAME' => $topic_info[0]['t_cache_first_title'],'POST' => $row['report_text'],'POSTER' => $poster));
