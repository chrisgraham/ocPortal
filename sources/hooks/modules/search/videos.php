...
class Hook_search_videos
{
	...
	function info()
	{
		...
		if ($GLOBALS['SITE_DB']->query_select_value('videos','COUNT(*)')==0) return NULL;
	...
	function run($content,$only_search_meta,$direction,$max,$start,$only_titles,$content_where,$author,$author_id,$cutoff,$sort,$limit_to,$boolean_operator,$where_clause,$search_under,$boolean_search)
	{
		...
		$rows=get_search_rows('video','id',$content,$boolean_search,$boolean_operator,$only_search_meta,$direction,$max,$start,$only_titles,'videos r',array('','r.description','r.title'),$where_clause,$content_where,$remapped_orderer,'r.*',array(),'galleries','cat',true);
