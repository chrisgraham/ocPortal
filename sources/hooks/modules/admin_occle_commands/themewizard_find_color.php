...
class Hook_themewizard_find_color
{
	/**
	...
	*/
	...
	{
		if ((array_key_exists('h',$options)) || (array_key_exists('help',$options))) return array('',do_command_help('themewizard_find_color',array('h'),array(true,true)),'','');
		else
		{
			if (!array_key_exists(0,$parameters)) return array('','','',do_lang('MISSING_PARAM','1','themewizard_find_color'));

			$input=$parameters[0];
			if (substr($input,0,1)=='#') $input=substr($input,1);
			if (strlen($input)==3) $input=$input[0].$input[0].$input[1].$input[1].$input[2].$input[2];
			...
			{
				if ($i<count($results)-10) continue;
