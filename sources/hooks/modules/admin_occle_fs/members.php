...
class Hook_members
{
	...
	function remove_file($meta_dir,$meta_root_node,$file_name,&$occle_fs)
	{
		...
		if (count($meta_dir)==1)
		{
			...
			require_code('ocf_members_action');
			require_code('ocf_members_action2');
			$field_id=$GLOBALS['FORUM_DB']->query_value_null_ok('f_custom_fields','id',array($GLOBALS['FORUM_DB']->translate_field_ref('cf_name')=>$file_name));
			if (is_null($field_id)) // Should never happen, but sometimes on upgrades/corruption...
			{
				...
				$field_id=$GLOBALS['FORUM_DB']->query_value('f_custom_fields','id',array($GLOBALS['FORUM_DB']->translate_field_ref('cf_name')=>$file_name));
			}
