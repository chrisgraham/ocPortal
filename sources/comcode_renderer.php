...
function comcode_parse_error($preparse_mode,$_message,$pos,$comcode,$check_only=false)
{
	...
	$form=do_template('FORM',array('_GUID'=>'207bad1252add775029b34ba36e02856','URL'=>$post_url,'TEXT'=>'','HIDDEN'=>$hidden,'FIELDS'=>$fields,'SUBMIT_ICON'=>'buttons__proceed','SUBMIT_NAME'=>do_lang_tempcode('PROCEED')));
...
function _do_tags_comcode($tag,$attributes,$embed,$comcode_dangerous,$pass_id,$marker,$source_member,$as_admin,$connection,&$comcode,$structure_sweep,$semiparse_mode,$highlight_bits=NULL,$on_behalf_of_member=NULL,$in_semihtml=false,$is_all_semihtml=false)
{
	...
	switch ($tag)
	{
		...
		case 'attachment_safe':
			...
			if ($attributes['type']=='extract') $attributes['type']='';
			// LEGACY
			...
			elseif ($attributes['type']=='island' || $attributes['type']=='left_island' || $attributes['type']=='right_island')
			{
				$attributes['framed']='1';
				$attributes['type']='';
				if ($attributes['type']=='left_island') $attributes['float']='left';
				if ($attributes['type']=='right_island') $attributes['float']='right';
			}
			...
			if (is_null($attachment_row))
			{
				...
				if ($connection->connection_write!=$GLOBALS['SITE_DB']->connection_write)
				{
					...
					if (url_is_local($attributes['thumb_url'])) $attributes['thumb_url']=get_custom_base_url().'/'.$attributes['thumb_url'];
				}
