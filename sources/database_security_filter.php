...
{
	$complex_keywords=array('ORDER','GROUP','AS','OR','NOT','LIKE','IN','BETWEEN','UNION','HAVING');
	...
	if (in_array(strtolower(trim($query_parts[0])),array('select','update','delete')))
	{
		...
		{
			if (in_array(strtoupper(trim($part)),$complex_keywords)) return false;
		}
		foreach ($complex_operators as $operator)
		{
			if (strpos($query,$operator)!==false) return false;
		}
		if (preg_match('/[a-z]\(/',strtolower($query))) return false; // SQL functions
		return true;
	}
	return false;
}
					...
					foreach ($query_call_strings as $query_call_string)
					{
						...
						if ($offset!==false) // First do a fast check on the line itself
						{
							...
							} else
							{
								...
								if (in_array($str,$_strings)) $ok=true;
							}
