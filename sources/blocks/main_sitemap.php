					...
					if (has_page_access(get_member(),$page,$zone))
					{
						...
						if (!is_null($__entrypoints[0]))
						{
							...
							if (is_null($entrypoints))
							{
								...
								if (($low_memory) && (!HIPHOP_PHP) && (strpos(file_get_contents(get_file_base().'/'.$path),' extends standard_crud_module')!==false)) // Hackerish code when we have a memory limit. It's unfortunate, we'd rather execute in full
								{
