	...
	function run($map)
	{
		...
		if (!is_null($members))
		{
			...
			foreach ($members as $bits)
			{
				...
				$users_online->attach(do_template('OCF_USER_MEMBER',array('_GUID'=>'a9cb1af2a04b14edd70749c944495bff','COLOUR'=>$col,'GID'=>strval($pgid),'PROFILE_URL'=>$url,'USERNAME'=>$username,'USERGROUP'=>$usergroup)));
				$num_members++;
			}
			if ($num_guests!=0)
			{
