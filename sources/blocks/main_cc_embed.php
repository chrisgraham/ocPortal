...
class Block_main_cc_embed
{
	...
	function info()
	{
		...
		$info['version']=2;
		$info['locked']=false;
		$info['parameters']=array('param','select','template_set','display_type');
		return $info;
	}
	...
	{
		$info=array();
		$info['cache_on']='array(array_key_exists(\'display_type\',$map)?$map[\'display_type\']:NULL,array_key_exists(\'template_set\',$map)?$map[\'template_set\']:\'\',array_key_exists(\'select\',$map)?$map[\'select\']:\'\',array_key_exists(\'param\',$map)?$map[\'param\']:db_get_first_id(),get_param_integer(\'max\',30),get_param_integer(\'start\',0))';
		$info['ttl']=60*2;
		return $info;
	}
	...
	function run($map)
	{
		...
		if ((!is_null($map)) && (array_key_exists('select',$map)))
		{
			...
			$select=ocfilter_to_idlist_using_db($map['select'],'id','catalogue_entries','catalogue_categories','cc_parent_id','cc_id','id');
		}
		...
		list($entry_buildup,,,)=get_catalogue_category_entry_buildup(is_null($select)?$category_id:NULL,$catalogue_name,$catalogue,'CATEGORY',$tpl_set,$max,$start,$select,$root,$display_type,true);
