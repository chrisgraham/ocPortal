...
function render_download_box($row,$pic=true,$include_breadcrumbs=true,$zone=NULL,$text_summary=NULL)
{
	...
	$description=(!is_string($row['description']) && !isset($row['description__text_parsed']))?get_translated_tempcode('download_downloads',$just_download_row,'description'):comcode_to_tempcode($row['description']);
...
function get_download_sub_categories($category_id,$root=NULL,$zone=NULL,$order=NULL)
{
	...
	foreach ($rows as $myrow)
	{
		...
		$description=get_translated_tempcode('download_categories',$myrow,'description');
