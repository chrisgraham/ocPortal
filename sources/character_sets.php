...
function convert_to_internal_encoding($data,$input_charset=NULL,$internal_charset=NULL)
{
	...
	if (((version_compare(phpversion(),'4.3.0')>=0) || (strtolower($internal_charset)=='iso-8859-1')) && ((strtolower($input_charset)=='utf-8') || (strtolower($input_charset)=='utf8')) && /*test method works...*/(will_be_unicode_neutered($data)) && (in_array(strtolower($internal_charset),array('iso-8859-1','iso-8859-15','koi8-r','big5','gb2312','big5-hkscs','shift_jis','euc-jp')))) // Preferred as it will sub entities where there's no equivalent character
	{
		$test=entity_utf8_decode($data,$internal_charset);
		if ($test!==false) $data=$test;
	}
	elseif ((function_exists('unicode_encode')) && ($internal_charset!='utf-8') && ($input_charset=='utf8') && ($VALID_ENCODING))
	{
		$test=@unicode_encode($data,$input_charset);
		if ($test!==false) $data=$test;
	}
	elseif ((function_exists('unicode_decode')) && ($internal_charset=='utf-8') && ($input_charset!='utf8') && ($VALID_ENCODING))
	{
		$test=@unicode_decode($data,$input_charset);
		if ($test!==false) $data=$test;
	}
	...
	elseif (((strtolower($input_charset)=='utf-8') || (strtolower($input_charset)=='utf8')) && (strtolower(substr($internal_charset,0,3))!='utf'))
	{
		$test=utf8_decode($data); // Imperfect as it assumes ISO-8859-1, but it's our last resort.
		if ($test!==false) $data=$test;
	}
