        ...
        foreach ($details['forum_groupings'] as $best => $forum_grouping) {
            if (array_key_exists('subforums', $forum_grouping)) { // We only show if there is something in it
                ...
                foreach ($forum_grouping['subforums'] as $subforum) {
                    ...
                    $edit_url = has_actual_page_access(get_member(), 'admin_ocf_forums') ? build_url(array('page' => 'admin_ocf_forums', 'type' => '_edit', 'id' => $subforum['id']), 'adminzone') : new Tempcode();
...
function ocf_render_topic($topic, $has_topic_marking, $pt = false, $show_forum = null)
{
    ...
    // Emoticon
    if ($topic['emoticon'] != '') {
        $emoticon = do_template('OCF_FORUM_TOPIC_EMOTICON', array('_GUID' => 'dfbe0e4a11b3caa4d2da298ff23ca221', 'EMOTICON' => $topic['emoticon']));
    } else {
        $emoticon = do_template('OCF_FORUM_TOPIC_EMOTICON_NONE');
    }
...
function ocf_get_forum_view($forum_id, $forum_info, $start = 0, $max = null)
{
    ...
    // Find topics
    $extra = '';
    if ((!has_privilege(get_member(), 'see_unvalidated')) && (addon_installed('unvalidated')) && (!ocf_may_moderate_forum($forum_id, $member_id))) {
