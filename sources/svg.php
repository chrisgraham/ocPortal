...
function _draw_segment($colour, $angle, $radius, $start_x, $start_y, $end_x, $end_y)
{
    if ($angle == 360) {
        ...
        return '<path d="M' . float_to_raw_string(X_PADDING + floatval($radius)) . ',' . float_to_raw_string(Y_PADDING + floatval($radius)) . ' L' . float_to_raw_string(X_PADDING + floatval($start_x)) . ',' . float_to_raw_string(Y_PADDING + floatval($start_y)) . ' A' . float_to_raw_string(floatval($radius)) . ',' . float_to_raw_string(floatval($radius)) . ' 0 ' . float_to_raw_string(($angle > 180) ? 1.0 : 0.0) . ',1,' . float_to_raw_string(X_PADDING + floatval($end_x)) . ',' . float_to_raw_string(Y_PADDING + floatval($end_y)) . ' Z" style="fill: #' . ($colour) . ';" class="pie_chart" />' . "\n";
    }
}
...
function create_pie_chart($data)
{
    ...
    if (($max_degrees < 355.0) || ($max_degrees > 365.0)) {
