		...
		CREATE INDEX `award_archive.a_type_id` ON ocp4_award_archive(a_type_id);
		ALTER TABLE ocp4_award_archive ADD FOREIGN KEY `award_archive.a_type_id` (a_type_id) REFERENCES ocp4_award_types (id);

		CREATE INDEX `award_archive.member_id` ON ocp4_award_archive(member_id);
		ALTER TABLE ocp4_award_archive ADD FOREIGN KEY `award_archive.member_id` (member_id) REFERENCES ocp4_f_members (id);

		CREATE INDEX `award_types.a_description` ON ocp4_award_types(a_description);
		ALTER TABLE ocp4_award_types ADD FOREIGN KEY `award_types.a_description` (a_description) REFERENCES ocp4_translate (id);

		CREATE INDEX `award_types.a_title` ON ocp4_award_types(a_title);
		ALTER TABLE ocp4_award_types ADD FOREIGN KEY `award_types.a_title` (a_title) REFERENCES ocp4_translate (id);
