		CREATE TABLE ocp10_import_parts_done
		(
		...
		CREATE TABLE ocp10_import_session
		(
		...
		CREATE TABLE ocp10_import_id_remap
		(
		...
		CREATE INDEX `import_id_remap.id_new` ON ocp10_import_id_remap(id_new);
		ALTER TABLE ocp10_import_id_remap ADD FOREIGN KEY `import_id_remap.id_new` (id_new) REFERENCES ocp10_anything (id);

		CREATE INDEX `import_id_remap.id_old` ON ocp10_import_id_remap(id_old);
		ALTER TABLE ocp10_import_id_remap ADD FOREIGN KEY `import_id_remap.id_old` (id_old) REFERENCES ocp10_anything (id);
