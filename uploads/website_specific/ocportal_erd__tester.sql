		...
		CREATE INDEX `test_sections.s_assigned_to` ON ocp_test_sections(s_assigned_to);
		ALTER TABLE ocp_test_sections ADD FOREIGN KEY `test_sections.s_assigned_to` (s_assigned_to) REFERENCES ocp_f_members (id);

		CREATE INDEX `tests.t_section` ON ocp_tests(t_section);
		ALTER TABLE ocp_tests ADD FOREIGN KEY `tests.t_section` (t_section) REFERENCES ocp_test_sections (id);

		CREATE INDEX `tests.t_assigned_to` ON ocp_tests(t_assigned_to);
		ALTER TABLE ocp_tests ADD FOREIGN KEY `tests.t_assigned_to` (t_assigned_to) REFERENCES ocp_f_members (id);

		CREATE INDEX `tests.t_inherit_section` ON ocp_tests(t_inherit_section);
		ALTER TABLE ocp_tests ADD FOREIGN KEY `tests.t_inherit_section` (t_inherit_section) REFERENCES ocp_test_sections (id);
