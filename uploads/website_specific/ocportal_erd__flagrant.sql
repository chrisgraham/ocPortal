		...
		CREATE INDEX `text.user_id` ON ocp6_text(user_id);
		ALTER TABLE ocp6_text ADD FOREIGN KEY `text.user_id` (user_id) REFERENCES ocp6_f_members (id);

		CREATE INDEX `text.the_message` ON ocp6_text(the_message);
		ALTER TABLE ocp6_text ADD FOREIGN KEY `text.the_message` (the_message) REFERENCES ocp6_translate (id);
