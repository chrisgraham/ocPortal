		...
		CREATE INDEX `text.user_id` ON ocp_text(user_id);
		ALTER TABLE ocp_text ADD FOREIGN KEY `text.user_id` (user_id) REFERENCES ocp_f_members (id);

		CREATE INDEX `text.the_message` ON ocp_text(the_message);
		ALTER TABLE ocp_text ADD FOREIGN KEY `text.the_message` (the_message) REFERENCES ocp_translate (id);
