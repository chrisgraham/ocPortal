		CREATE TABLE ocp10_community_billboard
		(
			the_message integer NOT NULL,
			days integer NOT NULL,
			order_time integer unsigned NOT NULL,
			activation_time integer unsigned NOT NULL,
		...
		CREATE INDEX `community_billboard.the_message` ON ocp10_community_billboard(the_message);
		ALTER TABLE ocp10_community_billboard ADD FOREIGN KEY `community_billboard.the_message` (the_message) REFERENCES ocp10_translate (id);

		CREATE INDEX `community_billboard.member_id` ON ocp10_community_billboard(member_id);
		ALTER TABLE ocp10_community_billboard ADD FOREIGN KEY `community_billboard.member_id` (member_id) REFERENCES ocp10_f_members (id);
